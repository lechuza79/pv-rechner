...
import { districtGeometry } from "../../lib/district-geometry";
...
/** Shared district page using the region register and existing monitor widgets. */
export default async function LandkreisSeite({ region, children, ranking, basePath, crumbs, stand, intro, state, variant = false }: {
  ...
  const shapes = await districtGeometry(region.region_id);
  const metrics = [
  ...
  const allPrograms = await getFundingPrograms();
  ...
  for(const town of towns) for(const p of matchFundingForAgs(allPrograms,town.region_id)) {
    if(p.level==="bund")continue;
    programs.set(p.id,p);
    coverage.set(p.id,[...(coverage.get(p.id)??[]),town.name]);
  }
  const foerderProgramme=[...programs.values()].map(programm=>({programm,standLabel:fundingStandLabel(programm),zaehlt:fundingZaehlt(programm),geltungsbereich:districtPrograms.some(p=>p.id===programm.id)?"Gilt im gesamten Landkreis":"Gilt in: "+coverage.get(programm.id)?.join(", ")}));
  ...
  return <><main className={`solar-page ${variant === "dark" ? foundation.foundation : ""} ${styles.page} ${variant ? styles.cutVariant : ""} ${variant === "dark" ? styles.darkVariant : ""}`} data-story-scheme={variant === "dark" ? "dark" : "light"}>
    ...
    <section className={styles.hero}>
      ...
      <div className={styles.heroHeading} data-map-hero-heading>
        ...
        <h1>{region.name}</h1>
        <p className={styles.lede}>{places.length.toLocaleString("de-DE")} Gemeinden.<br />Solarenergie im Überblick.</p>
      </div>
      {shapes.length>0 ? <RegionKarte shapes={shapes} metrics={metrics} /> : <p>Für dieses Gebiet liegt derzeit keine aktuelle Karte vor. Die Gemeindedaten stehen unten in der Übersicht.</p>}
      </div>
    </section>
    </div></div>
    <GemeindeAboDialog name={region.name} ags={region.region_id} verwaltungLabel="Für den Landkreis"/>
    ...
    <section id="atlas-ranking" data-widget-ranking className={`${styles.section} ${styles.raceSection}`} aria-label="Ranking">
      <DistrictRaceWidget name={region.name} stand={stand}
    ...
    <GemeindeSkripte navigationOnly daten={{districtOverview:true,ags:null,kreisAgs:region.region_id,kreisLabel:region.name,landAgs:state.id,landLabel:state.name,startArea:region.region_id,startKategorie:"count",klasse:"alle",kreisBase:basePath+"/",stufen:STUFEN,discoveries:[],name:region.name,liveUrl:`https://solar-check.io${basePath}`,genitiv:region.name,widgetUrl:"https://solar-check.io/energie-widgets"}}/>

  </main><div data-page-footer><SiteFuss zwischen={<DataSourcesSection><DataSourceNote label="Datenbasis:" source={[DATA_SOURCES.mastr, DATA_SOURCES.bkg, DATA_SOURCES.iconD2Archive, DATA_SOURCES.era5Archive]}/></DataSourcesSection>}/></div></>;
}
