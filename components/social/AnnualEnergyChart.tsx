...
export function AnnualEnergyChart({data,compact=false,ohneBedienung=false}:{data:EnergyYear;compact?:boolean;ohneBedienung?:boolean}){
 ...
 <svg viewBox={compact?radialPreviewViewBox(data.days.map((day,i)=>point(i,day.solarMwh+day.windMwh)),260,82):"0 0 520 520"} role="img" aria-label={`${hasWind?"Solar und Wind":"Solarerzeugung"} in ${data.town}, ${data.year}. Modellierte Tageserträge.${hasWind?" Wind mit vereinfachter Referenzkurve.":""}`}>
 ...
 {!compact&&<path d={segment(i,0,maximum)} stroke="transparent" strokeWidth="5" onPointerEnter={()=>setHover(i)} onPointerLeave={()=>setHover(null)} onClick={()=>setSelected(selected===i?null:i)}><title>{`${formatStoryDate(d.date)} · Solar ${Math.round(d.solarMwh)} MWh${hasWind?` · Wind ${Math.round(d.windMwh)} MWh`:""}`}</title></path>}
 </g>;})}
