...
export function InstallationCountChart({counts,powerShare,compact=false}:{counts:{total:number;selected:number;label:string};powerShare:number;compact?:boolean}){
 ...
 return <div className={`${styles.chart} ${compact?styles.compact:''}`}><div className={styles.splashes} aria-hidden="true">{[0,1,2].map(layer=><div key={layer} className={styles.splash} data-layer={layer} style={{['--splash-mask' as string]:`url(${brandAssets.splashMask})`}}/>)}</div>
