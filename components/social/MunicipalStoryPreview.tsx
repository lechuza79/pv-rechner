...
export default function MunicipalStoryPreview({stories,name,embedded=false,surfaceScheme='light'}:{stories:StoryConcept[];name:string;embedded?:boolean;surfaceScheme?:string}) {
 ...
 const visual=(story:StoryConcept,compact:boolean)=><div className={`${chart.visualTheme} ${styles.visual} ${story.kind==='yield'?chart.preview:''}`} data-story-scheme="dark"><MunicipalChart story={story} compact={compact}/></div>;
 const cards=(all:boolean)=><div className={all?styles.feed:styles.grid}>{(all?stories:stories.slice(0,3)).map((story,index)=><button className={styles.card} key={story.id} onClick={()=>open(index)}>{visual(story,true)}<div className={styles.copy}><small>{formatStoryDate(story.period)}</small><h3>{story.title}</h3><span className={styles.storyCta}>Story ansehen <IconArrowRight size={16}/></span></div></button>)}</div>;
