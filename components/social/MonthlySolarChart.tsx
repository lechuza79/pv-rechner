...
export function MonthlySolarChart({data,compact=false,autoPlay=false}:{data:SolarMonth;compact?:boolean;autoPlay?:boolean}){
 ...
 const chartViewBox=compact?radialPreviewViewBox(data.days.flatMap(day=>day.mw.map((v,i)=>point(i+.5,v))),280,90):"0 0 560 560";
 ...
 const backdropX=compact?280-viewSize:viewX;
 const backdropY=compact?280-viewSize:viewY;
 const backdropSize=compact?viewSize*2:viewSize;
 ...
 <svg viewBox={chartViewBox} role={compact?"img":"group"} aria-label={`Solarleistung in ${data.town??'Trier'}, ${formatStoryDate(data.month)}. ${data.days.length} Tageslinien, 24 Stunden. Modellierter Monatsertrag ${(data.totalMwh/1000).toFixed(2)} GWh.`}>
 <defs>
 ...
 {!compact&&[0,6,12,18].map(hour=>{const a=hour/24*Math.PI*2-Math.PI/2;return <text key={hour} x={280+Math.cos(a)*260} y={280+Math.sin(a)*260+5} textAnchor="middle" className={styles.hour}>{String(hour).padStart(2,'0')}{!compact&&hour===0?' Uhr':''}</text>;})}
