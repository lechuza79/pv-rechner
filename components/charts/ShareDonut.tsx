...
export function ShareDonut({values}:{values:DonutValue[]}){
 ...
 return <div className="sc-share-donut">
  ...
  <circle cx="110" cy="110" r="73" fill="none" stroke="black" strokeOpacity=".16" strokeWidth="2" pointerEvents="none"/></svg><div className="sc-share-center" aria-live="polite"><strong>{formatted}</strong><span>{unit}</span><small>{selected?.label??'Gesamt'}</small></div></div>
