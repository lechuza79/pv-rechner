 ...
 *
 * The two colours are pinned to literal values on purpose: the mark must stay
 * on-brand no matter what theme a host sets on the widget. Pinned is not the
 * same as typed, though — they come from the brand tokens (tokens[…], so a
 * literal hex lands in the SVG and survives the image export). Typed out, they
 * stayed the old blue when the mark went lime on 20.09.2026, and this badge sat
 * under every exported chart as the last blue thing on the page.
 */
function SolarCheckMark() {
  const marke = tokens["--color-brand"];
  const markeTief = tokens["--color-brand-deep"];
  return (
    <svg
      width={11}
      height={11}
    ...
    >
      <defs>
        ...
        <linearGradient id="sc-mark-grad" x1="10.6766" y1="19.2691" x2="10.6766" y2="6.11926" gradientUnits="userSpaceOnUse">
          <stop stopColor={marke} />
          <stop offset="1" stopColor={marke} stopOpacity="0" />
        </linearGradient>
      </defs>
