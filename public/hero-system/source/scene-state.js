...
export function sceneState(date,location,weather){
 ...
 const phase=alt<-6?'night':alt<12?(hour<12?'dawn':'dusk'):'day';
 ...
 return {phase,solarElevation:alt,cloud:clamp(weather.cloud/100),rain,snow,wind:clamp(weather.wind/15,0,2.4),direction:Math.sin((weather.direction??270)*Math.PI/180)*-1,
...
export function validateWeather(json){
 ...
 return {temp:c.temperature_2m,cloud:c.cloud_cover,wind:c.wind_speed_10m,direction:c.wind_direction_10m,code:c.weather_code,rain:c.rain+c.showers,asOf:c.time*1000};
}
