...
export function mountHeroStage({root,stage,scene,state,quality='auto',motion=true,layout={},assets={},sunStyle='natural',moonPhase='today'}){
 ...
 function dispose(){
  if(disposed)return;disposeContrast();disposed=true;events.abort();stop();cancelAnimationFrame(bootFrame);clearTimeout(scrollTimer);ro.disconnect();io.disconnect();renderer?.dispose();sun.dispose();night.dispose();
 ...
 listen(scene,'sc-contrast-request',wake);
