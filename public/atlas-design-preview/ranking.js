...
const ownIncluded=()=>within({population:9564});
...
let inviteTarget=null,inviteDismissed=false;
function dismissInvite(){inviteDismissed=true;updateInvite();}
...
function updateInvite(){
 const next=inviteDismissed?null:choices.querySelector('.ranking-undiscovered:not([aria-pressed="true"]):not(:disabled)');
...
window.addEventListener('pagehide',()=>inviteObserver.disconnect(),{once:true});
function render(animate=false){const m=metrics.find(m=>m.id===active),rows=getRows(m),me=ownRow(rows),total=area==='09679'?rows.length:(remote?.total||0);const id=++animationId;
const top=rows.slice(0,3),order=top.length===3?[top[1],top[0],top[2]]:top;
const previous=me?rows.find(r=>r.rank===me.rank-1):null;
section.dataset.share=JSON.stringify({title:m.title,rank:me?.rank??null,total,value:me?fmt(me.value):null,unit:m.unit,area:area==='09679'?'im Landkreis Würzburg':area==='09'?'in Bayern':'in Deutschland',classLabel:classes.find(c=>c[0]===classId)[1],owner:owner==='alle'?'alle Anlagen':owner==='privat'?'private Anlagen':'gewerbliche Anlagen und Freiflächen'});
const html=top.length?`<p class="atlas-kicker">Die Top ${Math.min(3,top.length)}</p><h3>${m.title}</h3><p class="ranking-detail">${owner==='alle'?'Private und gewerbliche Anlagen':owner==='privat'?'Private Anlagen':'Gewerbliche Anlagen und Freiflächen'} · ${m.unit==='Anlagen'?'Anzahl':m.unit}</p><div class="ranking-podium">${order.map(r=>`<div class="ranking-contender ${r.id==='09679147'?'is-own':''}" data-rank="${r.rank}">${r.href?`<a href="${escape(r.href)}" target="_blank" rel="noopener">${r.id==='09679147'&&r.rank<=3?`<img class="ranking-badge" src="/atlas-design-preview/rank-badges/${m.id==='storage'?'battery':'roof'}-${r.rank}.png" alt="Platz ${r.rank} – ${escape(m.title)}" width="96" height="96">`:``}${escape(r.name)}</a>`:`<span>${r.id==='09679147'&&r.rank<=3?`<img class="ranking-badge" src="/atlas-design-preview/rank-badges/${m.id==='storage'?'battery':'roof'}-${r.rank}.png" alt="Platz ${r.rank} – ${escape(m.title)}" width="96" height="96">`:``}${escape(r.name)}</span>`}<strong class="ranking-value" data-value="${r.value}">${fmt(r.value)}</strong><span class="ranking-unit" hidden>${m.unit}</span><div class="ranking-step" style="height:${top[0].value>0?160*r.value/top[0].value:0}px"></div></div>`).join('')}</div><div class="ranking-own"><span>Höchberg${me&&me.rank<=3?' · auf dem Podest':''}</span>${me?`<strong>Platz ${fmt(me.rank)} <small>von ${fmt(total)}</small></strong><span class="ranking-value">${fmt(me.value)} ${m.unit}</span>`:`<p>${ownIncluded()?'Keine eigene Platzierung verfügbar.':'Liegt außerhalb der gewählten Größenklasse (9.564 Einwohner).'}</p>`}</div><p class="ranking-gap">${me?.rank===1?'Höchberg führt diese Kategorie an.':previous?`${fmt(previous.value-me.value)} ${m.unit} Abstand zu ${escape(previous.name)} auf Platz ${previous.rank}.`:me&&top.length?`${fmt(top[0].value-me.value)} ${m.unit} Abstand zur Spitze (${escape(top[0].name)}).`:''}</p>`:'<h3>Keine Orte in dieser Vergleichsgruppe</h3><p>Wähle eine andere Ortsgröße oder ein größeres Gebiet.</p>';
...
function decoratePosition(button,metric,known){
 ...
 const rank=Number(known?.rank);if(!known&&!button.disabled){position.classList.add('has-badge');const placeholder=document.createElement('img');placeholder.className='ranking-list-badge';placeholder.src='/atlas-design-preview/rank-badges/rank-mystery.png';placeholder.alt='Platzierung noch nicht entdeckt';placeholder.width=96;placeholder.height=96;position.prepend(placeholder);}if(rank>=1&&rank<=3){position.classList.add('has-badge');const badge=document.createElement('img');badge.className='ranking-list-badge';badge.src=`/atlas-design-preview/rank-badges/${metric.id==='storage'?'battery':'roof'}-${rank}.png`;badge.alt=`Höchberg: Platz ${rank} – ${metric.title}`;badge.width=96;badge.height=96;position.prepend(badge);}
}
function revealChoice(){if(id!==animationId)return;const newlyRevealed=!seen.has(m.id);seen.set(m.id,{rank:me?fmt(me.rank):'—',denominator:me?' / '+fmt(total):'',caption:me?fmt(me.value)+' '+m.unit:ownIncluded()?'Keine Platzierung':'Andere Größenklasse'});const button=choices.querySelector(`[data-category="${m.id}"]`);if(!button)return;button.querySelector('.ranking-choice-rank').textContent=me?fmt(me.rank):'—';button.querySelector('.ranking-choice-denominator').textContent=me?' / '+fmt(total):'';button.querySelector('.ranking-choice-caption').textContent=me?fmt(me.value)+' '+m.unit:ownIncluded()?'Keine Platzierung':'Andere Größenklasse';
decoratePosition(button,m,seen.get(m.id));updateInvite();
...
if(me&&me.rank<=3){stage.querySelector('.ranking-own')?.remove();stage.querySelector('.ranking-gap')?.remove();}
else if(me){stage.querySelector('.ranking-own').innerHTML=`<strong class="ranking-inline-rank">${fmt(me.rank)}</strong><span class="ranking-inline-town">Höchberg<small>von ${fmt(total)} Orten</small></span><strong class="ranking-inline-value">${fmt(me.value)}<small>${m.unit}</small></strong>`;stage.querySelector('.ranking-own').classList.add('ranking-outside');}
...
update();
choices.innerHTML='';for(const metric of metrics){const available=area==='09679'||metric.id==='power',own=available?ownRow(getRows(metric)):null,b=document.createElement('button');b.type='button';b.className='atlas-rank-row ranking-choice';b.dataset.category=metric.id;b.disabled=!available;b.setAttribute('aria-pressed',String(active===metric.id));const known=seen.get(metric.id);b.innerHTML=`<span class="ranking-choice-position"><strong class="ranking-choice-rank">${known?.rank??'—'}</strong><small class="ranking-choice-denominator">${known?.denominator??''}</small></span><span><strong>${metric.title}</strong><span class="ranking-choice-caption">${known?.caption??(available?'Platzierung entdecken':'Nur im Landkreis verfügbar')}</span></span><span aria-hidden="true">→</span>`;b.onclick=()=>{pause();active=metric.id;render(true);section.querySelector(`[data-category="${metric.id}"]`).focus({preventScroll:true});};choices.append(b);decoratePosition(b,metric,known);}
updateInvite();
const next=section.querySelector('.ranking-next');const availableMetrics=metrics.filter(metric=>area==='09679'||metric.id==='power');next.hidden=availableMetrics.length<2;next.onclick=()=>{const index=availableMetrics.findIndex(metric=>metric.id===active);active=availableMetrics[(index+1)%availableMetrics.length].id;render(true);};
const areaLabel=area==='09679'?'im Landkreis Würzburg':area==='09'?'in Bayern':'in Deutschland';
...
section.querySelector('.ranking-intro-copy').innerHTML=`Wie steht Höchberg da? Wir vergleichen ${fmt(total)} Orte ${areaLabel} in der Größenklasse <span class="ranking-help-wrap"><button type="button" class="ranking-class-help" aria-expanded="false" aria-describedby="ranking-class-tip">${classLabel}</button><span id="ranking-class-tip" role="tooltip" hidden>Verglichen werden Orte mit ähnlicher Einwohnerzahl:<br>${classes.map(c=>escape(c[1])).join('<br>')}.<br>Höchberg hat 9.564 Einwohner.</span></span>. Berücksichtigt werden ${ownerLabel}.`;bindTooltip();updateReset();
section.querySelector('.ranking-scope').textContent='';

full.querySelector('summary').textContent=area==='09679'?`Alle ${total} Orte in dieser Kategorie ansehen`:`Rangliste ansehen · ${total>100?'Top 100 und eigene Position':total+' Orte'}`;
section.querySelector('.ranking-table').innerHTML=`<table><thead><tr><th>Platz</th><th>Ort</th><th>${m.unit}</th></tr></thead><tbody>${rows.map(r=>`<tr class="${r.id==='09679147'?'is-own':''}"><td>${fmt(r.rank)}</td><th scope="row">${escape(r.name)}</th><td>${fmt(r.value)}</td></tr>`).join('')}</tbody></table>`;if(!animate)schedule();else clearTimeout(timer);}
async function load(){delete section.dataset.share;seen.clear();const id=++requestId;pause();busy=true;animationId++;stage.getAnimations({subtree:true}).forEach(a=>a.cancel());remote=null;full.open=false;section.querySelector('.ranking-table').innerHTML='';choices.innerHTML='';stage.innerHTML='<p>Vergleich wird geladen …</p>';full.hidden=true;status.textContent='';section.querySelector('.ranking-intro-copy').textContent='Der gewählte Vergleich wird geladen …';updateReset();section.querySelector('.ranking-scope').textContent='';section.querySelector('.ranking-availability').textContent=area==='09679'?'':'Für Bayern und Deutschland liefert die bestehende Rangliste derzeit Solarleistung je Einwohner. Weitere Kategorien sind im Landkreis verfügbar.';if(area!=='09679')active='power';playLabel();try{if(area!=='09679'){const q=new URLSearchParams({gebiet:area,owner,klasse:classId});const r=await fetch('/ranking-data?'+q);if(!r.ok)throw Error('unavailable');const j=await r.json();if(id!==requestId)return;remote=j;}if(id!==requestId)return;busy=false;full.hidden=false;render(false);}catch(e){if(id!==requestId)return;busy=false;stage.innerHTML='<h3>Vergleich gerade nicht verfügbar</h3><p>Bitte erneut versuchen oder Landkreis Würzburg wählen.</p>';status.textContent='Die Live-Rangliste konnte nicht geladen werden.';const retry=document.createElement('button');retry.type='button';retry.textContent='Erneut versuchen';retry.onclick=load;stage.append(retry);schedule();}}
...
full.ontoggle=()=>{if(full.open)pause();};section.addEventListener('focusin',e=>{if(e.target!==play)pause();});section.addEventListener('pointerenter',()=>clearTimeout(timer));section.addEventListener('pointerleave',schedule);document.addEventListener('visibilitychange',schedule);const observer=new IntersectionObserver(([entry])=>{visible=entry.isIntersecting;schedule();},{threshold:.25});observer.observe(section);window.addEventListener('pagehide',()=>{clearTimeout(timer);observer.disconnect();requestId++;},{once:true});
