...
- **`/einspeiseverguetung-rechner`** — EEG-Satz + Lebenslauf-Rechnung (schon erhalten / noch ausstehend) nach Inbetriebnahme-Monat/Jahr. Sätze ab 30.07.2022 aus der Gesetzes-Kette (`feedInRatesForCommissioning` in `lib/feedin-config.ts`, amtlich verankert im Anker-Test); ältere Anlagen tragen den Satz aus dem Bescheid per InlineEdit ein (bewusst kein geratener Wert). Laufzeit-Ende = 31.12. des zwanzigsten Jahres (§ 25 EEG, `feedInEndIso`).
