...
- **Das Amortisations-Rennen (`components/charts/KostenrennenWidget.tsx`, Daten `lib/kostenrennen.ts`) rechnet mit der Amortisationsfunktion des Rechners, nie daneben.** EIN Haushalt, ohne und mit Anlage; gezeichnet wird, was die Anlage EINGEBRACHT hat (kumulierter Nutzen aus `calc()` ab null) gegen eine Ziellinie „Anschaffung"; der Monat, in dem die Kurve sie erreicht, IST die Amortisation, per Test festgenagelt. Vier Entscheidungen aus dem Bau (05.09.2026), jede nach einem Fehlgriff: **Ab null, nicht als Kosten** — in kumulierten Stromkosten startet der PV-Haushalt 14.000 € über dem anderen, und keine Achse zeigt dann den Winter; die Race-Chart-Vorlage des Betreibers startet jeden Läufer bei null. **Das Wetter ist echt, Monat für Monat** — `calc()` nimmt über `verlauf.monatsprofilImJahr` je Betriebsjahr die zwölf DWD-Monatswerte des wiederholten Kalenderjahrs (`lib/strahlungsjahre.ts`, erzeugt von `scripts/dwd-strahlung-sync.ts`, jährlich als Action), normiert auf das Mittel des 25-Jahres-Fensters, damit die Menge die des Rechners bleibt; welche Strahlung dem Referenzertrag absolut entspricht, ist NICHT belegt, belegt ist nur die Streuung. Ein Referenz-Monatsprofil für alle Jahre sah aus wie erfunden — „jedes Jahr komplett symmetrisch" (Betreiber). **Die Achsen laufen mit** (x bis heute, y bis knapp über den bisherigen Höchststand), sonst ist das erste Jahr ein Strich am Boden. **Kein zweites Chart, keine Reiter:** Ein Stromrechnungs-Streifen darunter und Aufstellungs-Reiter (Wetterjahre, Preissprünge) waren gebaut und wurden auf Ansage wieder ausgebaut — Wetter und Preis waren nie als Auswahl gemeint, sondern als Mittel, Bewegung in die eine Kurve zu bringen. Das Rennen bleibt nominal (Betreiber, 05.09.2026).
