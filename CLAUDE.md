...
- **`/photovoltaik-foerderung`** + `/[bundesland]` + `/[bundesland]/[stadt]` — Förderdaten in Supabase (`funding_programs`, `funding_checks`) über `lib/funding-data.ts` mit Code-Seed als Fallback; ISR 3600, Rechner via `/api/funding`, Sync `/api/funding/setup?resync=1`. Runbook `scripts/foerder-verify.md`.
  ...
  **Der Katalog soll VOLLSTÄNDIG werden, nicht stichprobenhaft** (Vorgabe des Betreibers, 18.08.2026). `npm run foerder:screen` geht alle Gemeinden mit erfasster Förderseite durch, legt je Gemeinde ein Verdikt ab (`funding_coverage`) und macht dort weiter, wo der letzte Lauf aufhörte — dasselbe Gedächtnisprinzip wie beim Arbeitsvorrat, aus demselben Grund: Ohne Ablage begänne jeder Lauf wieder bei den größten Städten und käme nie in die Tiefe. Stand: **971 einmal vollständig gescreent**, daraus 4 Programme aufgenommen. **Der Screener liest nicht, er sortiert vor** — er meldete eine Seite über Zuschüsse zu Verhütungsmitteln als PV-Treffer, weil das Wort im Navigationsmenü stand. Ein Eintrag entsteht erst, wenn jemand die Amtsseite wirklich gelesen hat; ein Screening-Zitat ist nie die Quelle für eine Zahl. **Die eigentliche Lücke sind nicht die geprüften, sondern die unerfassten**: Für rund 9.500 Gemeinden haben wir die Verwaltungs-Website, aber keine Förderseite — was dort aufgelegt wird, sieht niemand. Übergabe dazu in Memory `project_foerder_abdeckung`.
