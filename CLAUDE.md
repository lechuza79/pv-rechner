...
**Navigation, Zielbild vom 18.09.2026:** Tools · Förderung · Energiemonitor · Vor Ort · Wissen; separat Login. Für Organisationen ist vorbereitet, bleibt zum Start aber ausgeblendet, bis passende Zielgruppenseiten existieren. `public/shared-nav/nav-content.js` hält die gemeinsamen Inhalte und die eindeutige Bereichszuordnung, `nav.js` die Interaktionen. `SharedSiteHeader` verwendet beides auf React-Seiten mit bestehender Anmeldung und einer ohne JavaScript bedienbaren Ersatznavigation. Startseite und Simulation verwenden denselben Baustein direkt. Angebotscheck bleibt Warteliste innerhalb Tools. Lokale Einstiege zeigen weiter auf den bestehenden Atlas; keine neuen kommunalen Monitorseiten und keine URL-Umzüge. Die alte Kopfzeile ist am 20.09.2026 gelöscht; was die Markierung im Menü garantiert, steht unter „Kopfzeile: Layout gehört ins Stylesheet“. Dieser Menüstand ist visuell abgenommen und seit dem Release der neuen Startseite (19.09.2026) auf allen Seiten im Einsatz, zusammen mit der neuen Fußzeile aus `lib/site-fuss.ts`. Mobil lassen sich mehrere Bereiche unabhängig öffnen; auf Desktop bleibt nur ein Flyout offen.

**Suche in der Kopfzeile (live seit 24.09.2026):** Lupe neben dem Login, Klick öffnet ein Klappfenster mit dem Cursor im Feld; Treffer beim Tippen, Enter oder kein JavaScript führt auf `/suche` (noindex). Ein erkannter Ort (Name oder Postleitzahl) bekommt eine Karte mit Ortsseite, Förderung vor Ort und den Rechnern mit vorbelegter PLZ; alles andere wird gegen das Seitenverzeichnis gesucht. Eine Anfrage kann beides sein („Förderung Würzburg“) — Themenwort ist, was eine Seite trifft, der Rest ist Ort. Logik `lib/suche.ts`, Verzeichnis `lib/suche-verzeichnis.ts` (**gelesen** aus Menü und Ratgeber-Registry, nie eine dritte Liste; ergänzt werden nur Suchwörter je Pfad), Darstellung EINMAL in `public/shared-nav/search-content.js` für Klappfenster und Suchseite.
- **Der Schalter ist `SUCHE_IM_MENUE` in `public/shared-nav/nav.js`**, und er steuert auch den Link in der Ersatznavigation ohne JavaScript. Ausgeschaltet erscheint die Lupe nur auf `/suche` und mit `?suchvorschau=1` (gilt für den Tab).
- **Menüs und Suche hängen einen festen Abstand unter der Menüzeile, gemessen, nicht per Kopfzeilen-Höhe** (`--sc-panel-top`, gesetzt beim Öffnen): Startseite und Gemeindeseite geben ihrer Kopfzeile 32 px Innenabstand unten, dort stand das Menü 38 px statt 6 px unter dem Menüpunkt. **Ein Klick auf einen Link zu einer anderen Seite lässt Menü und Suche offen**, bis die Seite wechselt — sofort zu schließen sah aus wie ein verlorener Klick (am 21.09. schon einmal gebaut und zurückgenommen, weil damals der Dev-Server die verlorenen Klicks verursachte).
- **Die Gliederung der Treffer benutzt h3/h4, nie h2:** Die Startseite färbt jede h2 im Kopfbereich nach dem Himmel; „Ort“ stand dort weiß auf dem hellen Fenster (1,1:1) — gefunden vom Browser-Test, nicht im Code sichtbar.
...
**Menü-Markierung: welcher Menüpunkt auf welcher Seite leuchtet** — die Regeln stehen vollständig unter „Kopfzeile: Layout gehört ins Stylesheet"; hier nur, warum es sie gibt. Vier Fehler dieser Klasse sind real passiert, alle von außen unsichtbar (die Seite funktioniert, nur die Markierung fehlt): ein zu tiefes Präfix nach einem Umzug, Ratgeber mit Top-Level-Slug, die **nie** markiert wurden, eine Cluster-Seite, die im Menü ganz fehlte — und zuletzt ein Wächter, der all das an einer Datei prüfte, die keine Seite mehr ausliefert.
...
**Flow 1: Rechner (`/photovoltaik-rechner?direkt=1`)** — für „ich kenne meine Anlage schon". Ergebnis auf derselben Seite.

**Flow 2: Empfehlung (die nackte Adresse `/photovoltaik-rechner`, seit 21.09.2026 der Haupt-Rechner)** — für „was passt zu mir?". Endet über eine Zwischenansicht (Empfehlung, Warum, Alternativen) im Ergebnis mit einer „Warum diese Anlage?"-Sektion. Wer seine Anlage kennt, kommt über `?direkt=1` in Flow 1. Die alte Adresse `/pv-bedarf-berechnen` leitet dauerhaft weiter; ihre Antwort-Parameter (`EMPFEHLUNG_KEYS` in `lib/share-keys.ts`) bleiben auf der statischen Seite, nur Rechnungs-Parameter schieben die Anfrage auf den dynamischen Zwilling. **Beide Flows landen bewusst auf DERSELBEN Ergebnisseite** — das ist eine Architekturentscheidung, keine Zufälligkeit.
...
- **`/klimaanlage-stromkosten`** — Kühlkosten + Gerätevergleich (Monoblock / mobile Split / fest installiert), CO₂, PV-Deckung. Kühlbedarf weather-driven aus **Kühlgradstunden** (`/api/cooling-degree`), im Ergebnis umschaltbar zwischen Ø letzte 5 Sommer (Default), letztem Sommer und Projektion ~20 J. Die beiden Vergangenheitswerte sind einmal im Jahr aus ERA5 vorgerechnet (`npm run klima:kuehlgrad`, nach dem 1. Oktober); **die Projektion ist unser heutiger Wert × die Zunahme je 0,25°-Rasterfeld aus acht CMIP6-Modellen** (NASA NEX-GDDP-CMIP6, `lib/klima-projektion.ts`, gebaut von `npm run klima:projektion` — nach jedem `klima:kuehlgrad` neu laufen lassen, ein Test hält die Jahre aneinander). Jedes Modell wird nur mit sich selbst verglichen, über einen Trend durch 30 Sommer statt fünf gegen fünf (fünf Sommer eines Modells sind überwiegend Wetter — ein Modell zeigte so zufällig weniger Hitze in 20 Jahren). Die Modellwerte selbst nie zeigen: Ein 25-km-Feld gibt für Frankfurt 5 Hitzetage, die Station misst 19. Bis 18.09.2026 kam die Projektion aus gespeicherten Werten der Open-Meteo-Klima-Schnittstelle, deren freier Zugang nur nicht-kommerziell ist; `klima_cache` wird seitdem nicht mehr gelesen. Lizenz: CC BY 4.0 je Modellgruppe, nicht NASAs CC0 (zwei Legal-Judges, Belege `docs/quellen/nex-gddp-cmip6/`). Gemessen gegen 56 bzw. 32 DWD-Stationen (`npm run klima:kuehlgrad-gegen-messung`): ERA5 +13 % (2025) / −2 % (2024), ERA5-Land −14 % / −19 % — deshalb ERA5, und deshalb liegen die Werte seit 09/2026 rund ein Fünftel über den alten. Bundesland-Fallback. **Kein Wetterwert einer Seite kommt mehr von der freien Vorhersage-Schnittstelle** — Live-Wetter, Tageskurven, Sonnenanzeige und Hitzewelle lesen eigene Schnappschüsse (`lib/icon-d2-store.ts`), festgenagelt von `lib/__tests__/wetter-ohne-freie-schnittstelle.test.ts`. `lib/aircon.ts` + `lib/aircon-config.ts`, Runbook `scripts/klimaanlage-verify.md`.
...
- **Der Rechner** (`/balkonkraftwerk/rechner`) — Haushalt/PLZ → Ausrichtung → Set-Größe; der letzte Schritt **empfiehlt** das wirtschaftlich beste Set (`recommendBalkonSet`), bietet aber alle drei an. Ertrag = Modul-kWp × PVGIS-Ertrag × Ausrichtung, **gedeckelt am 800-W-Wechselrichter** (Drosselung sichtbar). Default **keine Einspeisevergütung**, Fixpreis-Sets statt €/kWp. **Fördercheck seit 02.09.2026** — er fehlte bis dahin ganz, obwohl 43 Programme im Katalog einen Balkon-Betrag ausrechnen können; bei einem Set um 500 € sind 100–200 € Zuschuss ein Fünftel bis Drittel des Preises und damit der größte Hebel auf die Amortisation. Dieselbe Karte wie im PV-Rechner, gespeist aus der PLZ, die der Rechner ohnehin für den Standort-Ertrag hat. Der editierbare Kaufpreis zeigt weiter den BRUTTO-Wert — der Abzug gehört in die Karte, nicht in die Zahl, die der Nutzer bearbeitet. Miete/Eigentum ist zusätzlich ein Rechtshinweis (privilegierte Maßnahme seit 2024). `lib/balkon.ts` + `lib/balkon-config.ts`, Runbook `scripts/balkon-verify.md`.
  ...
  **Kaufbare Sets am Ende des Rechners (Affiliate, seit 09.09.2026)** — `components/BalkonAngebot.tsx`, Rechnung `lib/shop-angebot.ts`, Abruf `lib/shop-solakon.ts` über `/api/shop/balkon`. Der erste Punkt im Projekt, an dem Geld fließt; die Zusage „kein Leadfunnel" bleibt davon unberührt (der Nutzer sieht erst sein Ergebnis, das Angebot steht darunter), die Zusage „keine Werbung" ist damit überholt.
  - **DIE REIHENFOLGE IST DER NUTZEN, NIE DIE PROVISION** (Betreiber, 19.08.2026) — sortiert wird nach dem Gewinn über die Lebensdauer, also nach derselben Größe wie `recommendBalkon`. Ein anderer Maßstab hier hieße, dass Empfehlung und Angebotsblock auf derselben Seite auseinanderlaufen. **Der Grundsatz steht sichtbar am Block**, nicht nur im Code: Eine Regel, die niemand nachprüfen kann, ist eine Behauptung. Nebenwirkung, die bleibt: Das oberste Set kann eine LÄNGERE Amortisation haben als das zweite — richtig gerechnet, erklärungsbedürftig im Bild.
  - **ERST ENTDOPPELN, DANN RECHNEN — BLOCKER.** Jede Bewertung ist eine volle Jahressimulation über 8.760 Stunden. Der Shop führt 294 Varianten, die sich überwiegend in Halterung und Kabellänge unterscheiden — Dinge, zu denen unsere Rechnung nichts zu sagen hat. Alle zu bewerten hieß 294 Simulationen im Browser des Nutzers: **Der Block erschien im Browser-Test gar nicht mehr, weil die Seite noch rechnete.** Übrig bleiben die Kombinationen aus Modulleistung und Speichergröße, rund zwanzig. Festgenagelt in `lib/__tests__/shop-angebot.test.ts` → „Rechenaufwand" — die Fehlerklasse ist nicht „falsch", sondern „zu langsam zum Benutzen", und die ist im Diff unsichtbar.
  - **EIN KAUFWEG IST EIN LINK, KEIN KNOPF — BLOCKER.** Er war einer: Der Knopf öffnete den Shop per Skript, weshalb im ausgelieferten HTML **keine einzige Shop-Adresse** stand. Drei Folgen, alle unsichtbar, weil die Seite dabei normal aussieht — die Partnerkennung war nirgends nachweisbar, „in neuem Tab öffnen" und Mittelklick taten nichts, ein Screenreader meldete kein Ziel. `rel="sponsored"` ist bei einem Provisionslink Googles Vorgabe und kann an einem Knopf gar nicht hängen. **Der zugehörige Test war die ganze Zeit grün, weil er die Datenschnittstelle befragte statt die Seite anzusehen** — dieselbe Klasse wie ein Wächter, der das Vorhandensein statt der Verwendung prüft.
  - **EIN PREIS IST DER BETRAG AN DER KASSE, KEINE GRÖSSENANGABE** (`preisTeile` in `lib/atlas-format.ts`). Zwei Fehler nacheinander, beide live: gestaffelt als „1,5 Tsd. €" (die Formatierung für Regions-Summen — eine Zahl, die im Shop niemand wiederfindet), und danach auf volle Euro gerundet, obwohl **alle 294 Preise auf ,99 enden** — wir verlangten durchgehend einen Preis, den der Händler nicht nimmt, und zwar nach oben. Für gerundete Beispielrechnungen bleibt `fmtEuroVoll` zuständig; dort IST die Rundung die Aussage.
  - **Produktbilder laufen über UNSEREN Server** (Bildoptimierung mit freigegebenem Fremd-Host in `next.config.js`), nie direkt eingebunden: Sonst ginge die IP-Adresse jedes Besuchers an den Shop, bevor er etwas angeklickt hat. Ein Browser-Test verbietet die direkte Einbindung — der Fehler wäre sonst unsichtbar, das Bild sähe genauso aus.
  - **Die Bildfreigabe des Händlers liegt vor** (09.09.2026, per Mail an den Betreiber: „verwende gerne die Bilder", dazu ein Ordner mit weiteren Aufnahmen; auf Rückfrage am 10.09.2026 ausdrücklich auch AUSSERHALB des Angebotsblocks, womit die Nutzung in Ratgebern gedeckt ist). Ohne sie wäre das Spiegeln fremder Produktfotos Vervielfältigung und öffentliche Zugänglichmachung (EuGH C-161/17 *Renckhoff*, Rn. 21 zur Kopie auf den eigenen Server, Rn. 36 dazu, dass freie Abrufbarkeit daran nichts ändert) — **die Mail ist der Beleg und gehört aufgehoben**, und sie deckt nur diesen Händler; ein zweiter Shop braucht seine eigene. `BILDER_FREIGEGEBEN` bleibt trotzdem stehen: Eine Erlaubnis ist widerruflich, und das ist der eine Ort, an dem sich das zurücknehmen lässt.
  - **Zwei Fehlgriffe rund um diese Freigabe, beide lehrreich.** Der Code behauptete zwischenzeitlich, die Anfrage sei gestellt — sie war es nie; **was nur im Kommentar steht, ist unbelegt, bis jemand nachsieht.** Und die Abnahme des Blocks („das sieht total gut aus") war ein Scherz des Betreibers, der die Seite gar nicht sehen konnte, weil kein Bild geliefert worden war — sie wurde als Freigabe genommen und ging live. **Eine Abnahme setzt voraus, dass der Betreiber die Sache wirklich vor sich hatte;** liefert man kein Bild, ist jede Zustimmung eine über etwas Ungesehenes.
  - **Die Kennzeichnung ist Pflicht, keine Höflichkeit** (§ 5a Abs. 4 UWG): Der kommerzielle Zweck muss erkennbar sein, „sofern sich dieser nicht unmittelbar aus den Umständen ergibt" — bei einem Block, der wie ein Rechenergebnis aussieht, ergibt er sich gerade nicht. „Anzeige · Provision bei Kauf" steht deshalb AM Block, ein Browser-Test hält es fest.
  - **Kommt der Abruf nicht durch, verschwindet der Block ganz.** Ein Kaufhinweis ohne Preis wäre schlechter als keiner, und der Rechner darüber ist vollständig — er braucht den Block nicht.
  - **Die Klicks kommen beim Händler an (seit 18.09.2026 im Partnerkonto gezählt).** Vom 10.09. an kam dort nichts an — kein Zuordnungs-Cookie, kein gezählter Klick; der Händler hat daraufhin **Kampagnen-Parameter** nachgereicht (`utm_source=affiliate`, `utm_medium=cpo`), die seitdem an unseren Links hängen. Am 18.09. zeigte das Partnerkonto die ersten Besuche, **vermutlich alle eigene Tests** (ein Gerät, ein Browser) — eine echte Bestellung ist damit noch nicht belegt. **Seinen Vorschlag, auf die Startseite zu verlinken, haben wir NICHT übernommen**: Wer auf ein durchgerechnetes Set klickt und auf der Startseite landet, muss es dort erst wiederfinden, und bei 24 Stunden Zuordnungsfenster ist jeder Zwischenschritt teuer. **Die Links tragen kein `noreferrer`** — sonst sieht der Händler jeden Besuch als „Direct" und uns als Quelle gar nicht; die Referrer-Policy der Site schickt quer über Domains ohnehin nur den Domainnamen, nie Pfad oder Rechnerwerte. Ein Browser-Test hält das fest.
  - **Das Zuordnungsfenster sind 24 STUNDEN, nicht 89 Tage** (Programmbedingungen Abschnitt 7, Stand 03/2019). Die 89 Tage gelten nur für die Bestellung, wenn das Produkt bereits im Warenkorb liegt; hineinlegen muss der Kunde es in der 24-Stunden-Sitzung, und die endet vorzeitig, sobald er über einen fremden Partnerlink zum Shop kommt. Für ein Produkt um 1.500 €, über das Leute wochenlang nachdenken, ist das knapp — wer den Block umbaut, sollte den Kaufweg nicht verlängern.
  - **Die Teilnahme am Partnerprogramm steht als eigener Satz am Block** — vertragliche Pflicht nach Abschnitt 10 der Programmbedingungen, die die UWG-Kennzeichnung NICHT miterfüllt: „Anzeige · Provision bei Kauf" nennt die Provision, nicht das Programm. Festgenagelt im Browser-Test.
...
- **`/photovoltaik-bestand-deutschland`** — die Bestandsfragen („wie viele Solaranlagen/Balkonkraftwerke gibt es in Deutschland", zusammen ~190 Suchen/Monat bei geringer Konkurrenz, gemessen 26.08.2026). Zahlen aus `lib/anlagenbestand-server.ts`, Ableitungen und Texte rein in `lib/anlagenbestand.ts`, Widget `components/charts/AnlagenbestandWidget.tsx` (Register-Eintrag `anlagenbestand`, Embed `/embed/anlagenbestand-deutschland`). **Unser Vorteil ist allein die Frische** — jede Jahresstatistik beantwortet dieselbe Frage —, deshalb steht der Datenstand des Registerauszugs an jeder Zahl und die Seite trägt in `lib/stand.ts` **nur Live-Werte, kein Prüfdatum**: Wir prüfen diese Zahlen nicht, wir holen sie.
  - **Der Vergleichszeitraum wird benannt, nicht behauptet — BLOCKER für alles, was auf dem Register rechnet.** Das Register führt je Anlage nur das **Jahr** der Inbetriebnahme; ein Bestand „vor zwölf Monaten" ist daraus **nicht** ableitbar, ein Jahresendbestand schon. Bis zum 26.08.2026 stand in einem Social-Post „in den letzten zwölf Monaten" über einem Vergleich gegen den 31.12. — bei Datenstand 5. August waren es sieben. `zeitraumSeitStichtag()` leitet den Wortlaut aus Datenstand und Stichtagsjahr ab, `SocialKennzahlen.stichtagJahr` reicht das Jahr durch, Tests verbieten die Zwölf-Monats-Form.
  - **Die Bundeszahlen kommen aus dem Rollup, nie aus der Summe der Gemeindezeilen.** Die Award-Tabelle filtert auf bewohnte Gemeinden mit Slug und verfehlt den Bund um rund 1.500 Anlagen (gemessen). In gerundeten Millionen fällt das nicht auf — und ist deshalb genau der stille Widerspruch, den eine Seite mit zwei Quellen für dieselbe Größe erzeugt. `socialKennzahlen()` speist sein `wachstum` seitdem aus `getNationalSolarStock()`; die Gemeindezeilen bleiben für den Einwohnerbezug (Stadt/Land, Länderdichte).
  - **Anteile unter einem Prozent tragen eine Nachkommastelle** (`anteilProzentFeinTeile`). 19.374 Freiflächenanlagen sind 0,3 % aller Anlagen; „0 %" daneben behauptet, es gebe sie nicht — dieselbe Fehlerklasse wie eine falsche Einheit. Im Bild kann das niemand nachfragen.
  - **Die Datengeschichten stehen als Absätze auf dieser Seite, nicht als eigene Seiten.** Alle Verteilungs-Anfragen („wo stehen die meisten Balkonkraftwerke", „solaranlagen pro einwohner bundesland") haben **null** Suchvolumen — gemessen, nicht geschätzt, und nicht noch einmal zu prüfen. `SocialPost.onsite` liefert dieselben Zahlen in einer Fassung ohne Ich-Form und ohne Quellenzeile.
  - **Gegen die Zubau-Seite abgegrenzt** (`lib/__tests__/bestand-zubau-abgrenzung.test.ts`): Zubau-Wörter sind im Auftritt der Bestandsseite verboten, Bestands-Wörter im Auftritt der Zubau-Seite. Beide leben auf denselben Registerdaten und wären ohne Trennlinie eine Dublette auf einer Anfrage.
- **`/photovoltaik-foerderung`** + `/[bundesland]` + `/[bundesland]/[stadt]` — Förderdaten in Supabase (`funding_programs`, `funding_checks`) über `lib/funding-data.ts` mit Code-Seed als Fallback; ISR 3600, Rechner via `/api/funding`, Sync `/api/funding/setup?resync=1`. Runbook `scripts/foerder-verify.md`. **Vorfälle und Messungen dieses Bereichs: `docs/lehren/foerder-katalog-2026-08.md`** — dort steht, warum die folgenden Regeln so lauten.
  ...
  **Eine TOTE Adresse ist ein Befund, kein Hindernis — und der abgelegte Zustand taugt nicht dafür (20.09.2026).** Das Abhaken verlangte einen Beleg AUS der Seite; eine Adresse, die mit 404 antwortet, kam deshalb nie aus dem Vorrat heraus. Gemessen an diesem Tag: **1.767 der 13.401 offenen Quellzeilen (13 %)** stehen auf Adressen, die der Seiten-Wächter selbst als unerreichbar führt. `--tot` hakt eine solche Zeile ohne Beleg ab und schreibt statt des Zitats die gemessene Antwort mit.
  - **Gemessen wird im selben Augenblick, nie aus dem Zustandsfeld.** Von 43 als unerreichbar geführten Adressen antworteten **30 am selben Tag mit HTTP 200** — darunter zwei, deren Name ein Förderprogramm verspricht. Wer nach dem Zustandsfeld abhakt, wirft zwei von drei lesbaren Förderseiten weg, ohne sie gesehen zu haben; das ist im Vorrat unsichtbar, weil die Zahl dabei genau so sinkt wie bei echter Arbeit. **`zustand: unerreichbar` ist eine Beobachtung von damals, keine Aussage über heute.**
  - **Nur `missing` (HTTP 404/410) sagt etwas über die QUELLE.** `blocked`, `shell`, `network` und `server` sagen etwas über unseren Versuch und lassen die Zeile stehen; antwortet die Adresse normal, gilt wieder die Belegpflicht. Der Grund kommt aus einem eigenen Fehlertyp des Lesers, nicht aus dem Zerlegen einer Fehlermeldung — eine Meldung ist Text und ändert sich beim nächsten Umformulieren still.
  - **JEDE Schreibweise muss „weg" ergeben, nicht irgendeine.** Genügte eine 404 unter den probierten Adressformen, hakte ein Tippfehler in der Erfassung die Zeile ab, während die richtige Form die Förderseite ausliefert. Festgenagelt in `lib/__tests__/funding-abhaken.test.ts` und `lib/__tests__/funding-source-reader.test.ts`, vor dem Einchecken fünfmal absichtlich kaputtgemacht.
...
- **Solar-Atlas** (Gemeinde-/Kreis-/Landesseiten aus MaStR) und **Ratgeber** (`lib/ratgeber.ts`) — Details in `docs/` und den Memory-Einträgen.
  ...
  **Das Anlagenregister führt VERALTETE Gemeindeschlüssel — BLOCKER (18.09.2026).** Eine Anlage behält den Schlüssel, unter dem sie gemeldet wurde; nach einer Fusion oder einem Kreiswechsel gehört er zu keiner Ortsseite mehr. Gemessen: 277 alte Schlüssel trugen rund 11.300 Anlagen (823 MW), verteilt auf 117 Orte — Hanau (seit 01.01.2026 kreisfrei) zeigte null Anlagen bei 98.000 Einwohnern, Berga-Wünschendorf und Uder weit zu wenig. Kein Fehler, kein roter Test; aufgefallen ist nur Hanau, weil seine Rangliste fehlte. Beide Importe übersetzen deshalb über `aktuellerGemeindeschluessel()` (`lib/ags-nachfolger.ts`), Kreis und Land kommen aus dem **neuen** Schlüssel. Die Zuordnung ist die amtliche Destatis-Liste der Gebietsänderungen (`npm run destatis:gebietsaenderungen`, erzeugt eine Datei im Repo — **nie von Hand, nie über Namensgleichheit**: es gab zwei Obergeckler nebeneinander). Teilausgliederungen ziehen nicht um; eine aufgeteilte Gemeinde geht an den Nachfolger mit den meisten Einwohnern. **Nach jeder Neuerzeugung** `npm run mastr:umschluesseln -- --schreiben`, sonst warten die Zahlen bis zum nächsten Monatsimport. Übrig bleiben 15 gemeindefreie Gebiete (Forste, Seen) — sie haben keine Ortsseite und zählen im Kreis mit.
...
Ergebnis: 10–90 %; angezeigt ganzzahlig (calcEigenverbrauch), gerechnet UNGERUNDET
(calcEigenverbrauchExakt) — die ganzen Prozent kippten im Geld stufenweise, eine
größere Anlage wies bis 1.418 € weniger Gewinn aus (Council 12.09.2026)
...
- **Das Stromkosten-Rennen (`components/charts/KostenrennenWidget.tsx`, Rechnung `lib/kostenrennen.ts`, Tagesverlauf `lib/kostenrennen-tage.ts`) rechnet mit der Amortisationsfunktion des Rechners, nie daneben.** Die DARSTELLUNG ist seit 05.09.2026 der geteilte Baustein `components/charts/RaceChart.tsx` — zwei Läufer, Ereignisse mit Erklärung, Texte und Formatierer herein, alles andere (Kamera, Tempo, Zeitleiste, Bild, Video) kommt aus dem Baustein. **Ein weiteres Rennen ist ein neuer Aufrufer, kein neues Chart**: Reihen aus dem Rechenkern, Register-Eintrag, Ereignisliste; wer am Baustein etwas ändert, ändert alle Rennen (Muster in `docs/race-chart-vorlage.md`). EIN Haushalt, ohne und mit Anlage; gezeichnet werden die aufgelaufenen Stromkosten beider (die Anlage startet mit ihrer Anschaffung vorn), Tag für Tag; wo die Linie ohne Anlage die andere kreuzt, ist die Anlage bezahlt — derselbe Monat wie die Amortisation des Rechners, per Test festgenagelt. Vier Entscheidungen aus dem Bau (05.09.2026), jede nach einem Fehlgriff: **Das Wetter ist echt, Monat UND Tag** — `calc()` nimmt über `verlauf.monatsprofilImJahr` je Betriebsjahr die zwölf DWD-Monatswerte des wiederholten Kalenderjahrs (`lib/strahlungsjahre.ts`), normiert auf das Mittel des 25-Jahres-Fensters, damit die Menge die des Rechners bleibt; innerhalb des Monats verteilt `lib/kostenrennen-tage.ts` den Monatsnutzen nach der Tagesstrahlung der DWD-Stationen (`lib/strahlung-tage.ts`), die Monatssumme bleibt exakt. Beide Reihen erzeugen `scripts/dwd-strahlung-sync.ts` und `scripts/dwd-strahlung-tage-sync.ts`, jährlich als Action. Ein Referenzprofil für alle Jahre sah aus wie erfunden — „jedes Jahr komplett symmetrisch" (Betreiber); Monatswerte allein blieben ein Sinus, weil der Jahreszyklus die Abweichungen erdrückt; erst Tage tragen die Textur echter Daten. Welche Strahlung dem Referenzertrag absolut entspricht, ist NICHT belegt, belegt ist nur die Verteilung. **Die Achsen laufen mit** — x vom Start bis heute (mindestens ein Jahr, am Ende alle 25 Jahre auf einer Breite; ein gleitendes Drei-Jahres-Fenster war gebaut und wurde auf Ansage zurückgenommen), y auf die Linien gepasst; in einer festen 0–40.000-€-Skala ist der Winter von Anfang an ein Strich. **Der Nutzen ab null war der falsche Weg**: Eine Fassung zeichnete, was die Anlage einbringt, und ließ den Haushalt ohne Anlage flach auf null liegen — eine Linie, die sich nicht bewegt, ist kein Läufer. **Kein zweites Chart, keine Reiter:** Ein Stromrechnungs-Streifen und Aufstellungs-Reiter (Wetterjahre, Preissprünge) waren gebaut und wurden auf Ansage wieder ausgebaut — Wetter und Preis waren nie als Auswahl gemeint, sondern als Mittel, Bewegung in die Kurven zu bringen. Das Rennen bleibt nominal (Betreiber, 05.09.2026). **Der Download ist ein VIDEO, kein Bild** (`lib/race-video.ts`): Die Animation ist der Inhalt, ein Standbild zeigt nur ihr Ende. Aufgenommen wird in Echtzeit über die Aufnahme-Schnittstelle des Browsers (WebM in Chrome/Firefox, MP4 in Safari; wo der Browser nicht aufnehmen kann, gibt es den Knopf nicht) — jeder Frame wird aus Kopf, Chart-SVG, Legende, Satz und Quellenzeile auf eine Leinwand gemalt, nicht vom Bildschirm abfotografiert (das dauert je Bild hunderte Millisekunden). Schneller als Echtzeit ginge nur mit Encoder und Container-Schreiber im Browser, also einer neuen Abhängigkeit — bewusst nicht.
- **Das Heizkosten-Rennen (`components/charts/HeizkostenrennenWidget.tsx`, Rechnung `lib/heizkostenrennen.ts`) ist der zweite Aufrufer des Race-Charts — und der Beweis, dass ein weiteres Rennen ein Aufrufer ist, kein Chart** (05.09.2026). Neue Gasheizung gegen Wärmepumpe im unsanierten Muster-Haus des Grüngas-Widgets auf derselben Seite (`/ratgeber/gasheizung-oder-waermepumpe#heizkostenrennen`, Embed `/embed/heizkostenrennen`). **Die Menge kommt aus dem Wärmepumpen-Rechner** — `calcHeatPump` legt dafür seine Jahreskosten je Seite offen (`kostenJeJahr`, dieselben Zahlen wie `years` und die TCO-Summen, nur nicht vorab verrechnet); das Rennen verteilt nur. **Die Form kommt vom Wetter:** Der Heizanteil eines Jahres folgt den Gradtagen (G20/15) des wiederholten Kalendertags aus den DWD-Tagesmitteltemperaturen (`lib/temperatur-tage.ts`, erzeugt von `scripts/dwd-temperatur-tage-sync.ts`, jährlich in derselben Action wie die Strahlung), die Jahre wiegen nach ihrer Gradtagzahl normiert auf das Fenster-Mittel; Warmwasser, Grundpreis und Wartung laufen gleichmäßig. Ohne Wetter endet das Rennen exakt beim Rechner und kreuzt in dessen Amortisationsjahr, mit Wetter bleibt die Menge auf 1,5 % beim Rechner und die Kreuzung im Jahr daneben — beides per Test festgenagelt (`lib/__tests__/heizkostenrennen.test.ts`). **Nicht gebaut, mit Absicht:** eine winterlich schlechtere Arbeitszahl — das wäre eine Modellannahme, die der Rechner nicht kennt; beide Seiten folgen derselben Gradtag-Form. **Betriebsjahr 1 ist hier das Startjahr**, nicht Startjahr + 1 wie im Stromkosten-Rennen: Der Rechner preist Jahr i mit dem Kalenderjahr `YEAR + i − 1`, und das Grüngas-Widget daneben beschriftet genauso — die Stufe 2029 muss im Rennen auf 2029 fallen. Der Race-Chart liest deshalb den Kalender aus `datumVon`, nicht aus Arithmetik; zwei weitere Bausteinfehler zeigte erst der zweite Aufrufer (Randmarke mit langer Zahl lief links aus dem Bild, k€-Achse doppelte sich beim engten Zoom).
...
Die Regel gilt weiter, der Anlassfall ist Geschichte. Die alte Kopfzeile entschied bis zum 18.08.2026 per Komponenten-Zustand, ob Desktop-Navigation oder Burger gerendert wird; der Server lieferte damit auf jedem Gerät die Desktop-Fassung, und auf schmalen Schirmen riss sie das Dokument über die Fensterbreite, bis die Hydratation korrigierte. Behoben wurde es damals mit einer Medienabfrage. **Seit dem Release der neuen Oberfläche am 19.09.2026 rendert diese Kopfzeile niemand mehr** — Menü und Umschaltpunkt liegen im Design-Paket (`public/shared-nav/nav.css` und `nav.js`), eingebunden über `components/SharedSiteHeader.tsx`. Ihre Regeln standen danach noch als tote Zeilen im Theme und wurden am 20.09.2026 entfernt, zusammen mit denen des alten Fußbereichs und der alten Vertrauens-Leiste.

- **Was bleibt: Ein Zustand im JavaScript darf nicht entscheiden, welche Navigation im HTML steht.** Wer eine Kopfzeile baut, liefert beide Varianten aus und blendet die falsche per Medienabfrage aus.
- **Gemessen wird gegen die Breite, die der Inhalt wirklich hat — nicht gegen den Umschaltpunkt.** Das ist der Unterschied zwischen formal eingehaltener Regel und funktionierender Seite: Ein einziger neuer Navigationspunkt brachte den Inhalt einmal auf 1162 px in einem Kasten von 1040 — seitlicher Überlauf auf JEDER Seite über den Breitenbereich der meisten Notebooks. **Der Hebel sind dabei die Beschriftungen, nicht der Umschaltpunkt**: Ihn anzuheben beseitigt den Überlauf, indem es die Navigation für die Mehrheit abschaltet — das repariert die Messung, nicht die Seite. Und **der Linux-Runner misst strenger als ein lokaler Browser** (gemessen 1326 gegen 1281 px, andere Schriftmetrik): „passt gerade" lokal heißt rot in der Prüfung.
- **Ein Test mit abgeschaltetem JavaScript beweist das Server-HTML, nicht das Layout.** Ohne JavaScript fehlen genau die Elemente, die die Zeile breit machen. `e2e/header-ohne-js.spec.ts` prüft deshalb beides: ohne JavaScript, dass der Server eine bedienbare Ersatznavigation schickt, und **mit** JavaScript über mehrere Breiten, dass nichts überläuft.
- **Overlay-Vorrang gehört an die Aktionsleiste, nicht an den `<header>`:** Abdunkelung und Schließen-Knopf liegen im selben Stapelkontext; den Header anzuheben verschiebt beide gemeinsam.

**Allgemein: Ein Test, der Formatierung vergleicht, ist in beide Richtungen wertlos.** Die erste Fassung des damaligen Umschaltpunkt-Tests verglich CSS-Zeilen als Zeichenketten. Gemessen: fünf harmlose Umformatierungen (ein Prettier-Lauf genügt) machten ihn rot, fünf echte Defekte ließ er durch — darunter eine gelöschte Regel, nach der es auf Mobil gar keine Navigation mehr gegeben hätte. Ein Unit-Test vergleicht **Zahlen und Vorhandensein**, das Verhalten prüft der Browser.

**Die alte Kopfzeile ist am 20.09.2026 gelöscht, und mit ihr der Wächter, der sie noch las.** `lib/__tests__/nav-aktiv.test.ts` hatte die Menü-Markierung aus der Datei als Text abgeleitet und war deshalb grün, ohne etwas zu sehen — dieselbe Klasse wie ein Test, der das Vorhandensein statt der Verwendung prüft. Mit ihr weg sind das Token `--header-max-width`, ihr Registry-Eintrag und die zwei Kopfzeilen-Regeln im Zahlen-Register von `scripts/claude-md-verify.ts` (beide lasen aus der toten Datei bzw. aus dem toten Token).

**Was das Skript der Startseite baut, steht ohne JavaScript trotzdem da — und die Texte werden GELESEN, nicht abgeschrieben (20.09.2026).** Ohne Skript trug die Startseite den Hero, das FAQ und die Fußzeile; Werkzeuge, Ortsdaten, Ratgeber und der Organisations-Abschnitt entstehen erst, wenn das Szenen-Bündel gelaufen ist. Jeder Crawler ohne JavaScript (die meisten KI-Crawler und die kleineren Suchmaschinen; Google rendert) las stattdessen den **Entwurf** aus dem Design-Paket — ältere Formulierung, zwei Einstiege statt fünf. Gemessen: 3.685 → 5.043 Zeichen Crawler-Text, +846 Byte komprimiert, und der Block kostet **null Pixel** (aus der laufenden Seite entfernt bleibt die Seitenhöhe gleich).
- **Der Ort ist die Entwurfs-Sektion selbst**, deren Inhalt die Übernahme durch eine Marke ersetzt. Sie ist das einzige Stück Körper, das das Skript ohnehin wieder wegnimmt (es setzt `hidden`) — damit ist mit Skript nichts doppelt und nichts sichtbar, ohne zweiten Versteck-Mechanismus, und der Sprungpunkt der Seite funktioniert weiter.
- **Die Wortlaute stehen NICHT im Code, sondern werden beim Übernehmen aus dem Bündel gelesen** (`scripts/startseite-sektionen.mjs` → `lib/startseite-sektionen.json`). Eine zweite Fassung driftet beim ersten Paket-Neubau, und beide Seiten sehen für sich richtig aus: Mit Skript zeigt die Seite die Fassung des Pakets, ohne Skript unsere, und nebeneinander sieht sie nie jemand. **Der erste Anlauf hat genau das gebaut — abgeschrieben plus Wächter dagegen — und der Betreiber musste darauf zeigen. Ein Wächter gegen Drift IST das Eingeständnis, dass eine zweite Quelle existiert.**
- **Gelesen wird beim ÜBERNEHMEN, nie beim Ausliefern.** Das Bündel ist verdichtetes fremdes JavaScript; die Extraktion taugt nur so viel wie ihre Muster. Beim Übernehmen bricht ein Fehlgriff den Lauf ab, während ein Mensch danebensteht — dieselbe Zusage, die dort jede Ersetzung ohnehin trägt. Im Auslieferungsweg wäre derselbe Fehlgriff die Startseite. **Beim Bauen genau so passiert:** Das Ende der Vorlage wurde als nächstes Rückwärts-Anführungszeichen genommen, die Vorlage schachtelt aber eigene — die Extraktion gab still weniger zurück, als sie behauptete.
- **Der Wächter prüft seitdem etwas anderes** (`lib/__tests__/startseite-statisch.test.ts`): dass die abgelegten Abschnitte noch zum Bündel passen (ein übernommenes Paket ohne neuen Auslese-Lauf lieferte sonst überholte Texte), dass alles Abgelegte auch auf der Seite ankommt, dass der Block in der ausgeblendeten Sektion sitzt und Text bleibt. Sechsmal absichtlich kaputtgemacht.
- **Die Simulation bleibt unangetastet** — ihr Entwurfs-Abschnitt steht unverändert; ihr Crawler-Text ist eine eigene Frage.

**Der Eintrag IST der Markierungs-Schlüssel — BLOCKER für jede neue Seite im Menü.** Die neue Navigation ordnet einen Pfad über `navigationOwner` (`public/shared-nav/nav-content.js`) **einer** von sechs Gruppen zu und sucht dann **nur innerhalb dieser Gruppe** den Eintrag, dessen Adresse exakt dem Pfad entspricht. Die früheren Schlüssel je Seite gibt es nicht mehr. Daraus folgen die zwei Bedingungen, die `nav-aktiv.test.ts` seitdem prüft — beide aus dem Dateibaum bzw. aus dem Menü selbst abgeleitet, nicht aus einer Liste:
- **Ein Eintrag, der nur in einer FREMDEN Gruppe steht, leuchtet nie.** Mehrere Einstiege auf dieselbe Seite sind ausdrücklich erwünscht (der Atlas steht unter „Energiemonitor" und unter „Vor Ort"), aber einer davon muss in der besitzenden Gruppe liegen. Geprüft wird das zweimal: am ausgelieferten Menü **und** an dem mit der vorbereiteten Gruppe „Für Organisationen" — sonst fällt ein Fehler dort erst auf, wenn jemand sie einschaltet. Der Unterschied ist real: Ein Kontakt-Link aus einer sichtbaren Gruppe heraus zeigt heute auf eine Gruppe, die gar nicht angezeigt wird, und markiert deshalb nichts.
- **Jede Seite eines Themen-Clusters bekommt überhaupt eine Gruppe**, auch die Artikel, die nicht im Menü stehen. Ohne Zuordnung ist auf der Seite kein Menüpunkt hervorgehoben, und man sieht ihr nicht an, wo sie hingehört.
- **Wer was prüft:** `lib/__tests__/shared-navigation.test.ts` hält die Zuordnung Pfad → Gruppe fest (benannte Fälle plus die Regel über alle Registry-Ratgeber, also auch „Förderung schlägt Ratgeber"), `nav-aktiv.test.ts` ausschließlich die Gegenrichtung. Eine dritte Fassung davon ist ein Fehler, kein Duplikat.

**Ob die Markierung am Ende wirklich gesetzt wird, sieht nur der Browser** (`e2e/header-ohne-js.spec.ts`): Gruppe hervorgehoben, der exakte Eintrag darin hervorgehoben, und **genau eine** Gruppe — zwei leuchtende Menüpunkte verbergen, wo die Seite wohnt. Ohne diesen zweiten Teil wäre der Umbau nur ein Ortswechsel derselben Blindheit gewesen: Der Unit-Test prüft die Regel, das Skript des Design-Pakets wendet sie an, und ein ausgebautes `aria-current` fällt in keinem Unit-Test auf.

**Die Menü-Einträge der beiden Balkon-Ratgeber sind dabei ersatzlos weg, und das ist die Regel, nicht ein Verlust.** Seit der Betreiber-Vorgabe vom 20.08.2026 gehört in das Menü, was direkt unter einem Bereich liegt (Startseite, Rechner, Förder-Überblick); was in einer Kategorie liegt, gehört in die Übersicht seiner Kategorie. Die Crawlbarkeit hängt davon nicht ab — die trägt die Fußzeile, und die prüft derselbe Test unverändert weiter.
...
**Die Fokus-Falle beim Nachbauen:** Der Mechanik-Effekt darf NICHT am `onClose`-Callback hängen (die Aufrufer übergeben eine frische Inline-Funktion pro Render) — sonst läuft sein Aufräumen mitten im Tippen und reißt den Fokus aus dem Eingabefeld. Deshalb `onCloseRef` + Effekt nur an `open`. Genau solche Details sind der Grund für den geteilten Baustein: es gab drei handgebaute Overlays, die sich in Fokus-Rückgabe, Tab-Falle, Scroll-Sperre und Mobil-Verhalten unterschieden. **Ausgenommen ist bewusst das Burger-Menü der Kopfzeile** (`public/shared-nav/nav.js`): ein Navigations-Flyout, kein Dialog, und es kommt als Ganzes aus dem Design-Paket — Öffnen, Schließen, Fokus und Scroll-Sperre bringt es selbst mit. Es auf den Dialog-Baustein umzubiegen hieße, das Paket an dieser Stelle zu gabeln.
...
**`components/FlowNav.tsx` ist der Standard für jeden Schritt-Flow** (Betreiber-Vorgabe 05.08.2026): Kein Schritt startet mit einer Vorauswahl · ein Klick auf eine Option **wählt nur aus**, er springt nicht weiter · der Weiter-Button ist ausgegraut, bis eine gültige Auswahl existiert · **Zurück sitzt immer links, Weiter immer rechts** — **der erste Schritt hat in keinem Rechner ein Zurück** (Betreiber, 22.09.2026; aus dem Flow heraus führt die Kopfzeile), Weiter bleibt dort trotzdem rechts. Die Auto-Advance-Variante (Klick auf Option springt direkt) existiert als zentraler Schalter `FLOW_ADVANCE_ON_SELECT` im Baustein — sie wird nie pro Seite gebaut, sondern nur dort umgelegt.
...
**Landkreis-Seiten (Release für alle Landkreise freigegeben 25.09.2026):** Die abgenommene dunkle Landkreis-Seite wird unter ihrer bestehenden kanonischen Adresse ausgeliefert. Die beiden Vorschau-Adressen leiten dauerhaft dorthin um. Deutschland und Bundesländer bleiben unverändert; kreisfreie Städte leiten weiterhin auf ihre Ortsseite. Gemeinsame Monitor-Komponenten, vollständige Gemeinde-Aggregation und 3D-Karte sind in `docs/landkreis-vorschau.md` dokumentiert. **Die Kreisseite liest je Landkreis EIN vorberechnetes Paket, nie ihre Gemeindepakete** (seit 25.09.2026): gebaut im Monatslauf direkt nach dem Upload der Gemeindepakete und täglich vom Sicherungslauf `kreis-pakete.yml` nachgezogen, veröffentlicht nur als vollständige Generation. Mitglieder eines Kreises bestimmt eine Regel für Seite und Bau (Register ohne gemeindefreie Gebiete und ohne aufgelöste Gemeinden); Betrieb und Wiederherstellung dort im Abschnitt „Precomputed district packages“. **Länder und Deutschland bekommen im selben Lauf und in derselben Generation je ein eigenes Paket** (`lib/region-package.ts`, seit 26.09.2026): summiert aus den Kreispaketen und den kreisfreien Städten mit den unveränderten Kreisfunktionen, ein unvollständiges Kind macht die Ebene „nicht verfügbar“, nie kleiner. Orte ohne jede Anlage im Register zählen seitdem als Null statt als Lücke — nur mit Bestätigung durch das Register beim Bau. Schnittstelle und Lücken: `docs/land-bund-energiemonitor-daten.md`.
...
**Kein Text verschwindet auf seinem Grund — BLOCKER (23.09.2026).** `e2e/kontrast.spec.ts` + `e2e/kontrast.ts`, eigener CI-Job, lokal `npm run test:kontrast`. Der Browser misst auf 37 Seiten × 2 Tagesstufen, was beim Leser ANKOMMT, nachdem alle Lagen übereinanderliegen und alle Variablen aufgelöst sind. Vor dem Fix 164 unlesbare Stellen auf 10 Seiten, danach null.
- **Der Anlass ist die Fehlerklasse, nicht der Einzelfall.** Die Designumstellung vom 20./21.09.2026 hat die BEDEUTUNG der Farbtoken geändert, nicht ihre Verwendungsorte: „Text auf Akzent" war vorher Weiß und ist jetzt eine fast schwarze Tinte. Wer Weiß von Hand getippt hatte, stand danach auf dem Kopf — die markierte Ranglisten-Zeile trug ihren Ortsnamen bei 1,3:1, ihre Einheiten bei 1,1:1, und das Flächengrün stand site-weit bei 1,8:1 auf hellem Grund. **Kein Typfehler, kein roter Test, kein kaputtes Aussehen:** Die Seite sieht aufgeräumt aus, sie ist nur an diesen Stellen leer.
- **Ein statischer Abgleich der Token im Quelltext kann das NICHT** — am selben Tag null Treffer, weil die Fälle über Zwischenvariablen, geerbte Klassen und getippte Werte zustande kamen. Gemessen wird deshalb die Wirkung im Browser.
- **BEIDE Enden der Tagesstufen, nie die gerade geltende.** Die sieben Stufen unterscheiden sich nicht nur im Grund; Textfarben, Signalfarben und Ränder werden je Stufe eigens gesetzt. Der erste Lauf prüfte nur die helle und übersah dadurch 53 schwarze Diagramm-Beschriftungen, die es nur nachts gibt.
- **Ein Zustand, der erst durch eine Eingabe entsteht, wird hergestellt.** Der Lauf setzt einen Heimatort, sonst gibt es die markierte Zeile gar nicht — und genau sie war der Anlass. Ein Test, der Seiten nur AUFRUFT, misst die halbe Oberfläche.
- **Nachgeprüft wird am BILD, nicht am Baum — BLOCKER.** Hinter Text kann ein Verlauf, ein Foto oder eine gezeichnete Fläche liegen; die hat keine Hintergrund-FARBE, und der Messkopf sieht dann den Grund der Seite statt den, auf dem der Text wirklich steht. Genau daraus entstand der erste Fehlalarm (die Gemeinde-Kopfzeile). Fotografiert wird das **Element** — ein Ausschnitt des Fensters scheitert an allem unterhalb der Falz, und **ein übersprungener Befund sieht aus wie ein behobener**: Mit dieser Fassung blieb der Lauf bei absichtlich eingebauter Lücke grün. Was sich nicht fotografieren lässt, wird deshalb GEMELDET, nicht übersprungen.
- **Gemessen wird, bis sich nichts mehr ändert**, nicht nach fester Frist: Manche Farbe steht erst fest, wenn ein Skript sie gesetzt hat. Eine feste Frist ist hier immer entweder zu kurz (erfundene Befunde) oder für jede ruhige Seite zu lang.
- **Drei Messfehler, die dabei erst Befunde erfanden und nicht zurückkommen dürfen:** Farbwerte in der 0..1-Schreibweise als 0..255 gelesen (ein heller Grund wurde zu fast Schwarz, 10 erfundene Funde) · Text in Diagrammen über die Schriftfarbe statt über die Füllfarbe gemessen, die dort gilt (53 erfundene Funde auf einer Seite) · Schriftgröße null bei einem Aufklapp-Dreieck. Jeder Farbwert läuft seitdem durch eine 1×1-Leinwand, Diagramm-Text bleibt bewusst draußen.
- **Die Grenze steht auf 3:1, nicht auf dem Standard** — mit ausgeschriebenem Grund und Frist im Test. Sie fängt „unlesbar", nicht „schlecht lesbar"; wer sie anhebt, hebt sie für den Bestand an und braucht eine eigene Runde.
- **Die Tagesstufe wird im Testlauf FESTGENAGELT.** Derselbe Test war auf der Hauptlinie zeitabhängig rot: Die Palette folgt der Sonne, und ein einzelnes kantengeglättetes Pixel traf die Toleranz mal und mal nicht. Wer eine Farbe prüft, pinnt die Stufe.

**Eine hervorgehobene Fläche ist ein eigenes Token** (`--color-bg-raised`, je Tagesstufe). Sie war vorher die Akzentfarbe mit heller Tinte darauf — und genau diese Paarung kippte bei der Umstellung. Eine Karte, die sich vom Grund abheben soll, hebt sich über Fläche, Rand und Schatten ab, nicht über eine Signalfarbe: Die Signalfarbe trägt eine Bedeutung, und „das ist deine Zeile" ist keine.
...
- **„✓ aktuell" steht nur innerhalb der Prüffrist** (seit 21.09.2026). Die Frist ist die aus `lib/pruefstand.ts` — dieselbe, gegen die `stand:faellig` meldet; jede taggenaue Zeile nennt dafür ihr `feld`, `standSeite()` rechnet das letzte gültige Datum aus, ein Test hält beides aneinander. Entschieden wird im Browser nach dem Laden: Eine statisch ausgelieferte Seite trägt sonst das „aktuell" ihres Bautags weiter, und Server und Browser stritten sich beim ersten Zeichnen über das Datum. Die Auszeichnung ist eine Werbeaussage auf jeder Rechnerseite; ohne echte Frist erscheint sie nicht.
...
npm run kommunen:klicks                # Wer hat den Brief geöffnet: Besuche je angeschriebener Gemeindeseite
npm run laender:sync  # Länderreihen aus Embers Jahresdatensatz neu erzeugen (läuft monatlich als Action)
npm run strahlung:sync # Gebietsmittel der Globalstrahlung je Monat aus den DWD-Rastern (jährlich als Action; das Wetter im Stromkosten-Rennen)
npm run strahlung:tage # Tagesstrahlung als DWD-Stationsmittel (jährlich als Action; die Tagesform im Stromkosten-Rennen)
npm run temperatur:tage # Tagesmitteltemperatur als DWD-Stationsmittel (jährlich als Action; die Gradtage im Heizkosten-Rennen)
npm run kfw:import    # KfW-Förderreport einlesen (jährlich, braucht poppler; --trocken misst nur)
npm run stats:erfassen # Tokens, Arbeitszeit und Nachrichten je Tag festhalten (wöchentlich als Auftrag; --schreiben legt ab)
npm run kosten:erfassen # Was das Projekt an Geld gekostet hat, aus der Buchhaltung (--bilanz zeigt die Gesamtübersicht)
```
...
**Die Auszeichnungs-Liste wird VORBERECHNET, nicht gecacht — BLOCKER (09.09.2026, dritter Anlauf).** Die Ja/Nein-Frage „hat dieser Ort eine Auszeichnung" hing an einer Rangfolge über alle 10.742 Orte, also 3,7 s Rechnung. Zwei Anläufe haben die Häufigkeit gesenkt und denselben Rest gelassen: **ein prozess-lokales Memo** zahlt den ERSTEN Aufruf jeder frischen Function (08.09.), **ein geteilter Cache mit Stundenfrist** zahlt ihn beim Fristablauf (09.09.: drei rote Gesundheitschecks, jedes Mal die erste Stichprobe bei 7,2 / 7,2 / 7,6 s an drei Orten in drei Bundesländern, 0,8 s vor der Notbremse — bei durchweg normalen Datenbankzeiten von 176–187 ms). Seit dem 09.09. entsteht die Liste im **Datenlauf** und liegt als Tabelle; der Seitenaufbau liest EINE Zeile über den Schlüssel (gemessen 76–136 ms statt 3.700). **Der Unterschied ist „meistens schnell" gegen „zuverlässig schnell", und für einen Outreach-Schub zählt nur das zweite.**
- **Wer hier wieder etwas cacht statt es vorzuberechnen, baut den dritten Anlauf desselben Fehlers.** `lib/__tests__/auszeichnung-vorberechnet.test.ts` hält deshalb die BAUWEISE fest, nicht eine Frist: Der Seitenaufbau darf den Index nicht bauen, egal mit welchem Zwischenspeicher davor. Eine Frist festzunageln hätte den dritten Anlauf gar nicht verlangt, sondern nur eine größere Zahl.
- **Erst schreiben, dann aufräumen.** Bricht der Aufbau dazwischen ab, stehen zu viele Orte in der Tabelle statt zu wenige — ein Platzhalter zu viel ist harmloser als eine halb leere Liste.
- **Der Ausfall ist von außen unsichtbar** (die Seiten antworten normal, nur ohne Platzhalter), deshalb prüft der Gesundheitscheck Bestand und Alter der Tabelle; älter als 45 Tage heißt, der monatliche Datenlauf ist zweimal ausgefallen.
...
- Festgenagelt von `lib/__tests__/auszeichnung-vorberechnet.test.ts`: geprüft wird die **Verwendung** (was der Seitenaufbau wirklich aufruft), nicht das Vorhandensein eines Caches — vor dem Einchecken viermal absichtlich kaputtgemacht, jedes Mal rot, auf dem Rückweg wieder grün.
...
4. **CDN-Cache-Header** auf `/api/weather` (s-maxage=900, liest nur den stündlichen Wetter-Schnappschuss) und `/api/pvgis` (s-maxage=2592000).
...
Tokens, Arbeitszeit, Nachrichten und Bestand je Kalendertag in `projekt_statistik` und `projekt_bestand` (Tabellen über `/api/projekt-statistik/setup`, RLS an ohne Policy). Rechnung in `lib/projekt-statistik.ts`, Erfassung `npm run stats:erfassen`, wöchentlicher Auftrag `solar-check-projekt-statistik`.

**Der Anlass ist ein Datenverlust, den niemand bemerkt hat (09.09.2026).** Claude Code räumt die Gesprächsprotokolle nach dreißig Tagen weg; die erste Projekthälfte (22.03. bis 14.07.2026) ist damit unwiederbringlich fort — kein Time Machine, keine Systemschnappschüsse, und die lokale Telemetrie führt **keine** Tokenzahlen (alles am 09.09.2026 nachgesehen, nicht erneut zu prüfen). Der Lauf muss **lokal** laufen: Die Protokolle liegen auf dem Rechner des Betreibers und nirgends sonst.

- **Zwei Herkünfte, nie vermischt.** Jede Tageszeile sagt, ob sie GEMESSEN ist oder GESCHÄTZT. Die Rückrechnung für März bis Juli lief **einmalig** (`--rueckrechnen`) und darf nicht wiederholt werden — ein zweiter Lauf schriebe mit einem neuen Kennwert andere Zahlen, ohne dass sich an den Daten etwas geändert hätte. Geschätzte Tage werden **vor** den gemessenen geschrieben, damit eine Messung nie von einer Schätzung überschrieben wird; ein Test hält die Reihenfolge fest.
- **Die Arbeitszeit ist eine VEREINIGUNG, keine Summe — BLOCKER.** An diesem Repo laufen regelmäßig bis zu elf Arbeitsstände gleichzeitig. Addiert kamen für 48 Tage **662** Stunden heraus (knapp 14 am Tag), zusammengelegt **261**. Die Zahl sieht in beiden Fällen normal aus.
- **Ein Protokoll-Zeitstempel ist ein ZEITPUNKT, kein gemeinter Tag.** `tagVon()` rechnet ihn auf den deutschen Kalendertag um; `tagInBerlin(iso)` wäre hier falsch, weil es einen String bewusst nur abschneidet. Zwischen 22:00 und Mitternacht deutscher Zeit steht in der Weltzeit noch der Vortag — eine Abendschicht läge auf dem falschen Tag. Die Tagesgrenze wird **gesucht**, nicht gerechnet: Sie liegt im Sommer zwei, im Winter eine Stunde vor Mitternacht Weltzeit, und beide Regime sind per Test festgenagelt.
- **Der laufende Tag wird nicht abgelegt** (dieselbe Regel wie bei der Kostenwache: nur vollständige Tage).
- **Codex zählt mit, getrennt geführt** (seit 09.09.2026). Am Projekt arbeiten zwei Werkzeuge; wer nur eines erfasst, nennt eine zu kleine Zahl. Zwei Unterschiede, die man nicht übersehen darf: Codex meldet seine Tokenzahl **kumulativ** je Sitzung (wer alle Meldungen addiert, zählt eine Sitzung mit hundert Zügen hundertfach — genommen wird die letzte), und sein zwischengespeicherter Anteil steckt **innerhalb** der Eingabe statt daneben. Gefiltert wird über das **Arbeitsverzeichnis** der Sitzung, nie über das Vorkommen des Projektnamens im Text. Gemessen 31.08. bis 08.09.2026: 181 Mio. Tokens in 125 Sitzungen an 7 Tagen — die Protokolle reichen nicht weiter zurück.
- **Die ARBEITSZEIT gehört dem Tag, nicht dem Werkzeug — BLOCKER.** Sie steht in einer eigenen Tabelle, über beide Werkzeuge zusammengelegt. Je Werkzeug abgelegt und dann summiert, käme dieselbe Stunde zweimal heraus: Wer nebeneinander eine Claude- und eine Codex-Sitzung offen hat, arbeitet trotzdem nur eine Stunde. Dieselbe Fehlerklasse wie bei den parallelen Arbeitsständen, nur eine Ebene höher.
- **Zurückgerechnet wird nur für Claude.** Die Codex-Protokolle beginnen am 31.08.2026; für März bis Juli einen Codex-Anteil hochzurechnen hieße, Arbeit zu erfinden, die es nicht gab.
- **Gemessen 15.07. bis 08.09.2026:** 45,6 Mrd. Tokens (davon 44,2 Mrd. wiedergelesener Kontext, 92 Mio. selbst geschrieben), 263 Stunden an 47 Tagen (mit Codex), 4.135 getippte Sätze mit Mittellänge 132 Zeichen, 104.383 Antworten, 60.806 Werkzeugschritte. Hochgerechnet für März bis Juli: 11,0 Mrd. Tokens und 62 Stunden.

**Der Aufwandsvergleich („wie lange hätte ein Team gebraucht") rechnet nach GEWERKEN, nicht nach Codezeilen — BLOCKER** (`lib/aufwand-schaetzung.ts`). Das übliche Verfahren aus Zeilen (COCOMO, Boehm 1981) liefert hier 62 Personenjahre gegen tatsächlich rund ein Drittel Personenjahr, also **Faktor 200**: Kein Verfahren ist so genau, dass so ein Ergebnis etwas belegt — es zeigt, dass das Modell nicht passt (es unterstellt handgeschriebenen Code ohne fertige Bausteine, und unsere 237.000 Zeilen enthalten Datentabellen, erzeugte Reihen und 50.000 Zeilen Tests). **Ein moderneres Werkzeug gibt es nicht**: Die gängigen Code-Zähler rechnen darunter alle dasselbe Modell von 1981, und der anerkannte Nachfolger (COSMIC-Funktionspunkte, ISO/IEC 19761) verlangt einen zertifizierten Menschen, der Funktionen von Hand zählt. Stattdessen die Lieferbestandteile zählen und jedem Erfahrungswerte in Personentagen geben, wie eine Agentur ein Angebot rechnet — mehrere hundert Personentage, rund ein Faktor **zehn** gegen die tatsächliche Arbeitszeit.
- **Die MENGE ist gezählt, die TAGE sind Urteil**, und beides steht getrennt: Die Summe wächst mit dem Projekt mit, und niemand hält die Schätzung für eine Messung. Genannt wird eine **Spanne**, nie ein Punktwert.
- **Die aktuelle Zahl steht hier bewusst NICHT.** Sie wächst mit jedem Gewerk mit und war binnen zwei Wochen um ein Sechstel gestiegen (600 → 711 Personentage), während an dieser Stelle weiter die alte stand — genau die Drift, gegen die `npm run claude-md:verify` gebaut ist. Wer sie braucht, lässt `npm run kosten:erfassen -- --bilanz` laufen.

### Was es an Geld gekostet hat

Monatssummen je Anbieter in `projekt_kosten`, der Listenwert der Rechenleistung je Tag und Modell in `projekt_listenwert` (beide über dieselbe Setup-Route wie die Statistik). Regeln und Aufteilungsschlüssel in `lib/projekt-kosten.ts`, Preise in `lib/modellpreise.ts`, die Gegenüberstellung in `lib/projekt-bilanz.ts`, Erfassung `npm run kosten:erfassen` — lokal, weil die Buchungsunterlagen auf dem Rechner des Betreibers liegen.

**DREI GRÖSSEN, DIE NEBENEINANDERSTEHEN UND NICHT DASSELBE SIND — BLOCKER.** *Bezahlt* ging vom Konto und steht in der Buchhaltung (Euro, gemessen). *Listenwert* ist, was dieselbe Rechenleistung über die Schnittstelle gekostet hätte — kein Geld, ein Vergleichsmaßstab (Dollar, gerechnet). *Herstellwert* ist, was ein Team für dasselbe Ergebnis verlangt hätte — weder Geld noch Messung (Euro, geschätzt). Die Bilanz bildet deshalb **keine Gesamtsumme**; ein Test verbietet ein Feld, das eine behauptet.

**Ein Verhältnis aus zwei verschiedenen Zeiträumen ist falsch, auch wenn beide Zahlen stimmen.** Die Kosten reichen über die ganze Laufzeit, der Listenwert nur über die noch vorhandenen Gesprächsprotokolle — rund vier Wochen. Die erste Fassung teilte beide Gesamtsummen und kam auf 17 statt 37; das sah plausibel aus und war um mehr als das Doppelte daneben. Der Rechenleistungs-Hebel entsteht deshalb **nur** aus den Monaten, für die beide Zahlen vorliegen, und nennt ihre Zahl.

**Der ANTEIL wird gerechnet, nicht abgelegt.** In der Tabelle steht der volle Monatsbetrag; welcher Teil auf dieses Projekt entfällt, entscheidet der Schlüssel im Code. Wäre der fertige Teilbetrag abgelegt, ließe sich ein später korrigierter Schlüssel nicht rückwirkend anwenden — und die alten Zeilen behaupteten weiter eine Genauigkeit, die es nie gab. Jeder Schlüssel trägt **Herkunft, Beleg und Messdatum**; ein Anteil von 100 % ist nur zulässig, wo die Zuordnung eindeutig ist. Gemessen sind sie über den Anteil an der Rechenleistung (Abo und Kontingente), über die Betriebsstunden je Datenbank, über die Einzelrechnungen des Registrars und über eine Dashboard-Messung (Auslieferung, die schwächste der Zahlen); geschätzt bleiben Mailversand und Quellcode-Verwaltung, und beide sagen das.

**Der Listenwert braucht das MODELL, nicht nur die Tokens.** Zwischen dem günstigsten und dem teuersten verwendeten Modell liegt beim Eingabepreis das Zehnfache, beim Wiederlesen das Vierzigfache. Eine Tagessumme ohne Modell ließe sich später keinem Preis mehr zuordnen — und die Protokolle sind nach dreißig Tagen weg. Ein unbekanntes Modell ergibt **keinen** Wert statt eines geratenen.

**Gelesen wird nur, was hierher gehört.** Die Buchungsunterlagen enthalten Honorare, Kundennamen, Steuernummern und private Abhebungen; abgelegt wird ausschließlich eine Monatssumme je bekanntem Anbieter, alles andere wird verworfen, bevor etwas geschrieben wird. Bewusst nicht aufgenommen sind Wurzelserver, Mobilfunk, Festnetz und Arbeitsplatz-Werkzeuge: Die Grenze verläuft nicht bei „geschäftlich", sondern bei „ohne dieses Projekt gäbe es die Ausgabe genauso".

**Zwei Fallen beim Einlesen, beide von außen unsichtbar und beide eingetreten.** macOS legt Dateinamen in ZERLEGTER Form ab — das „Ü" in „Übersicht" ist dort zwei Zeichen, im Quelltext eines; der erste Lauf meldete „0 Buchungen" und sah aus, als gäbe es die Dateien nicht. Und in einer Tabellendatei wird eine leere Zelle gar nicht erst geschrieben: Wer die Zellen einer Zeile durchzählt, statt ihre Adresse zu lesen, bekommt ab der ersten Lücke jede Angabe in der falschen Spalte. Beides festgenagelt in `lib/__tests__/projekt-kosten.test.ts`, vor dem Einchecken fünfmal absichtlich kaputtgemacht.

**Der Herstellwert rechnet mit einem ROLLENMIX, nicht mit einem Einheitssatz** (`lib/rollensaetze.ts`). Ein Projekt dieser Art baut keine Person allein; jedes Gewerk trägt deshalb eine Mischung aus CTO/Architekt, Senior und Junior, und die Mischung entscheidet über die Summe stärker als jeder einzelne Satz — zwischen reiner Fleißarbeit und reiner Konzeptarbeit liegt beim Mischsatz das Anderthalbfache. Die Anteile sind Urteil und stehen als solches da; sie müssen je Gewerk auf eins aufgehen, was ein Test prüft (ein Mix, der das nicht tut, liefert eine plausible Summe für zu wenige oder zu viele Tage und fällt sonst nirgends auf).

**Die Beleglage der Sätze ist dünn, und das gehört an die Zahlen.** Es gibt genau EINE belastbare Erhebung — den Freelancer-Kompass 2026 (über 5.400 Befragte; Median Software-/Webentwicklung 90 €/h bei n=356, alle IT 103 €/h, am 23.09.2026 gelesen) —, und die schlüsselt **nicht nach Seniorität** auf. Alles, was nach „Junior 80–100, Senior 120–160" aussieht, stammt aus Marktbeobachtungen von Agenturen über ihre eigenen Preise; die Spannen widersprechen sich zwischen den Quellen um bis zu 40 Punkte. Die Erhebung ist deshalb der Anker, Aufschlag und Spreizung sind ausgewiesenes Urteil.
- **Die Erhebung sagt etwas über die Spreizung, das gegen eine breite Staffel spricht:** nach Alter gestaffelt 80 € (20–30), 95 € (31–40), 96 € (41–50), 95 € (51+) — rund ein Fünftel Abstand, nicht das Doppelte. Ein Test hält die Rollenstaffel deshalb unter Faktor zwei; wer sie weiter spreizt, beschreibt eine Preisliste und keinen Markt.

**Ein Team, das dasselbe HEUTE bauen würde, arbeitet mit KI — und der Abschlag steht JE GEWERK** (`lib/ki-wirkung.ts`). Ohne ihn vergleicht die Aufstellung die eigene Arbeit mit einem Angebot von gestern. Gerechnet werden −40 % bei gleichförmiger Arbeit mit sofort prüfbarem Ergebnis (Inhaltsseiten, Schnittstellen, Tests, Bausteine), −25 % bei Umsetzung mit eigener Fachlichkeit, −10 % bei Modell, Recht, Architektur und Betrieb; über alle Gewerke rund −27 %. Die klassische Schätzung bleibt daneben sichtbar — der Abschlag ist die unsicherste Annahme der ganzen Aufstellung, und wer die Grundlage nicht sieht, kann sie nicht prüfen.
- **Die Messungen dazu widersprechen sich fundamental, und alle vier stehen im Code** — auch die, die gegen die Annahme spricht: METR 2025 fand im kontrollierten Versuch mit echten Aufgaben in vertrautem Code **19 % langsamer**, Google im Unternehmen **21 % schneller**, GitHub im Labor **55 %**, McKinsey bis zum Doppelten. Wer sich eine davon aussucht, sucht sich das Ergebnis aus; ein Test verlangt deshalb, dass mindestens eine Messung mit umgekehrtem Vorzeichen in der Liste steht.
- **Der bemerkenswerteste Befund gilt jeder Zahl in diesem Feld, auch unserer:** Dieselben Entwickler, die 19 % langsamer waren, schätzten sich hinterher auf 20 % schneller — 39 Punkte Abstand zwischen Gefühl und Messung, nach der eigenen Erfahrung. Eine aus dem Bauch gegriffene Produktivitätszahl ist zuverlässig zu optimistisch.
- **Kein Abschlag darf über dem Laborwert liegen** (Test), und ein pauschaler Faktor ist ausgeschlossen: Er behauptete, eine Rechtsrecherche im Volltext profitiere so stark wie eine Reihe gleichförmiger Schnittstellen. Der Test prüft die Staffel selbst, nicht nur, dass zwei Ergebnisse auseinanderliegen — eine erste Fassung blieb grün, als zwei der drei Stufen gleichgesetzt wurden.

**Die eigene Zeit ist NICHT ausschließlich dieses Projekt, und das steht jetzt an der Zahl.** Die Vereinigung legt parallele Arbeitsstände desselben Projekts zusammen; wer zwischen zwei Projekten hin- und herspringt, wird in beiden voll gezählt. Gemessen am 23.09.2026: **118 von 410 Stunden (29 %) liefen gleichzeitig mit einem anderen Projekt.** Die Erfassung schreibt diesen Anteil seitdem je Tag mit, und gerechnet wird mit den parallelen Stunden zur Hälfte — die neutrale Annahme bei zwei gleichzeitig offenen Projekten. „410 Stunden" hieße sonst stillschweigend „Stunden, in denen dieses Projekt offen war", und der Unterschied ist fast ein Drittel.

**Die eigene Arbeitszeit zählt als Einsatz mit.** Sie ist mit Abstand der größte Posten (Angabe des Betreibers: 140 € je Stunde, 23.09.2026; seine Ausgangsrechnungen 2026 weisen 100 € aus — deshalb steht sie als Angabe da, nicht als Messung). Das Verhältnis „Herstellwert je investiertem Euro" rechnet gegen Rechnungen **und** Zeit; die Zahl ohne die eigene Zeit steht daneben und ist rund fünfundzwanzigmal so groß. Wer nur sie nennt, rechnet die Hauptleistung heraus.
...
- **Behebung:** `.github/workflows/claude-autofix.yml` liest den strukturierten Messbericht unabhängig vom GitHub-Laufergebnis. Technische Befunde fordern Analyse an; die Tagesbremse zählt tatsächlich gestartete Modellschritte. Claude grenzt ein, behebt, lässt `tsc` + Tests laufen, misst am lebenden System nach und committet.
...
**Ein MONATLICHER Lauf wird gegen seinen TERMIN beurteilt, nie gegen ein Alter — BLOCKER (09.09.2026).** Der Import des Anlagenbestands aus dem Marktstammdatenregister fiel am 05.09.2026 aus: Der Server der Bundesnetzagentur war vom GitHub-Läufer aus nicht erreichbar, acht Abrufversuche brachen auf Netzwerkebene ab, der Lauf war nach 26 Sekunden rot. Vier Tage lang zeigten über 11.000 Gemeindeseiten die Zahlen des Vormonats; bemerkt hat es ein Mensch, zufällig.
- **Die Aufsicht gab es bereits und sie hat geschwiegen — das ist der eigentliche Befund.** Sie urteilte über das ALTER in Tagen (ab 45 gelb, ab 70 rot). Am Tag des Fehlschlags war der Bestand 31 Tage alt, also grün; gelb wäre er am 19.09. geworden (und Gelb erzeugt keine Nachricht), rot am 14.10. — da hätte der Oktober-Lauf die Lücke längst stillschweigend geschlossen. **Eine Tagesschwelle KANN „ein Lauf ist ausgefallen" nicht ausdrücken:** Sie misst den Abstand zum letzten Erfolg, nicht den zum letzten Termin. Dieselbe Klasse wie ein Wächter, der grün meldet, ohne etwas zu sehen.
- **Das Urteil kommt jetzt aus dem ZEITPLAN der Action** (`lib/mastr-import-plan.ts`, festgenagelt von `lib/__tests__/health-check-mastr-frische.test.ts`), gelesen aus der Workflow-Datei statt ein zweites Mal geführt: Wer den Termin verschiebt, verschiebt beides zugleich. Ist der Zeitplan nicht lesbar, meldet der Check GENAU DAS — eine Aufsicht, die sich beim ersten Tippfehler lautlos abschaltet, ist von keiner nicht zu unterscheiden.
- **Zwei Signale, weil eines allein zu spät ist.** Der Termin ist beweiskräftig, aber erst nach dem letzten Nachhol-Anlauf; der Ausgang des letzten Laufs ist sofort da, allein aber kein Beweis (ein roter Lauf kann vom nächsten Termin geheilt werden). Deshalb: roter Lauf bei offenem Zyklus → Warnung, verstrichener Zyklus → Befund an Claude. **Der Autofix darf hier nichts versuchen** — er fasst die Datenbank nicht an, und der 3-GB-Export wird dort nicht geladen.
- **Der Zeitplan hat seit dem 09.09.2026 drei Termine (5., 7., 9.), und der Lauf überspringt sich selbst.** Zusätzliche Termine ohne diese Frage wären keine Absicherung, sondern dreifache Arbeit — dreimal 3,2 GB und dreimal zwei Stunden Aufwärmen. **Bei jedem Zweifel wird GEARBEITET, nicht übersprungen** (Datenbank nicht erreichbar, Zeitplan nicht lesbar): Ein überflüssiger Lauf kostet eine halbe Stunde, ein fälschlich übersprungener einen Monat Daten. Übersprungene Läufe erklären auch keine Seiten für ungültig und wärmen nichts auf.
- **Ein Netzwerkfehler ist kein 404, und die Diagnose muss zum Symptom passen.** Die alte Meldung schob jeden Fehlschlag auf eine veraltete Schema-Version — die war in Ordnung, die Datei lag unter genau diesem Namen da. Eine 404 ist eine ANTWORT („diesen Tag gibt es nicht") und gehört zum normalen Rückwärtslaufen durch die Tage; ein abgebrochener Verbindungsversuch ist gar keine Antwort und wird wiederholt (vier Anläufe mit wachsender Pause, Suche wie Download). Kam auf keinen Versuch eine Antwort, sagt die Meldung genau das und nennt die Schema-Version nicht.
...
**Meldelogik — Benachrichtigung nur bei echtem Handlungsbedarf** (Vorgabe des Betreibers: „nur benachrichtigung wenn ich was tun muss"). Vier Stufen; Messbefund, Reparaturauftrag und Benachrichtigung sind getrennt (Betriebsbeschreibung: `docs/monitoring/health-incidents.md`):
- **selbst repariert** → Protokollzeile und eigenständige Auslieferung der Korrektur, keine Nachricht. Lokal bedeutet Exit-Code 2 „repariert".
...
- **muss Claude anschauen** → Messbericht bleibt ROT und fordert Autofix an. Die GitHub-Fehlermail entsteht nur bei einer neuen Eskalation: echte Ausfälle, Sicherheitsfehler und kaputte kritische Prüfungen sofort; andere technische Ursachen bei der dritten Beobachtung. Eine zweite langsame, frische Atlas-Probe aus anderen Regionen bestätigt den Befund sofort. Vorfälle werden nach Ursache gespeichert; Erholung und Wiederauftreten bleiben unterscheidbar (`lib/health-incidents.ts`, `lib/__tests__/health-incidents.test.ts`). Wiederholungen desselben eskalierten Vorfalls erzeugen keine neue Fehlermail; fehlende Messdaten gelten nicht als Erholung.
- **muss der Betreiber entscheiden** → einmalige Mail über `/api/alert` für den offenen Vorfall, ohne zusätzliche GitHub-Fehlermail. Nur Fälle mit mehreren vertretbaren Antworten oder persönlichem Kontozugang: War das Absicht? Geld ausgeben? Produkt/Priorität? Zugang erneuern?
...
- **Die BESUCHERSTATISTIK ist erreichbar — das Werkzeug dafür ist kaputt, nicht der Zugang (BLOCKER, 02.09.2026).** Das MCP-Werkzeug `get_web_analytics` antwortet auf **jeden** Aufruf mit „Web Analytics not found" (HTTP 404), auch mit richtiger Projekt- und Team-Kennung und auch beim zweiten Projekt des Kontos. Die dokumentierten REST-Adressen (`/v1/query/web-analytics/{visits,events}/{count,aggregate}`) antworten mit denselben Kennungen und demselben Zugang mit HTTP 200. Gelesen wird deshalb über `lib/web-analytics.ts`; der Zugang kommt aus `VERCEL_TOKEN` oder, wenn das fehlt, aus der Anmeldung der Vercel-Kommandozeile auf diesem Rechner — damit braucht eine Sitzung keine `.env`-Datei anzufassen. **Nicht mit der Kostenfrage verwechseln:** „Observability Plus" sperrt die Abrechnungs-Metriken, nicht die Besucherstatistik. Die Frage „kommen wir an die Zahlen?" war mehrfach mit „nein" beantwortet worden; sie ist mit „ja" beantwortet und wird nicht erneut aufgemacht.
  - **Je Bundesland fragen, nie über den ganzen Atlas.** Vercel liefert höchstens 100 verschiedene Adressen und wirft den Rest in einen Sammelposten — in einem Zug gefragt lagen darin 195 Besucher, also gerade die Gemeinden mit wenigen Aufrufen, um die es geht. Nach Ländern getrennt bleiben 9 übrig, und die werden benannt.
  - **„bis heute" schneidet heute ab.** Ein Datum ist eine Tagesgrenze; ein Ereignis von heute sah damit aus wie eines, das es nie gab. Der Vorgabewert ist deshalb **morgen**.
  - **Zwei Schlüssel, beide ohne Zutun am Brief: die ADRESSE sagt WELCHE Gemeinde, der VERWEIS sagt WAS passiert ist.** Jede Gemeinde hat ihre eigene Seite, und die verweisende Domain trennt den Klick aus dem Postfach von der Veröffentlichung (`npm run kommunen:klicks`, Einordnung in `lib/outreach-herkunft.ts`). **Der Verweis schlägt die Backlink-Suche:** Die kannte zwei Veröffentlichungen, die Besucherstatistik zeigte mehr. Ein Beitrag in einem sozialen Netz ist kein Backlink, den ein Verzeichnis crawlt.
  - **Stand 22.09.2026, jede Zeile am Beitrag selbst geprüft:** belegt sind Nidda (eigene Meldung, übernommen von wetterau.news und gude.news, alle mit Link), Wallertheim (Dorf-App), Berkenthin (eigene Website ohne Link plus Regionalzeitung „Herzogtum direkt" mit Link), Riedstadt (eigene Meldung und Rhein Main Verlag, beide mit Link) und Heringen (Antwort der Stadt und Besucher ab 28.08.; der Artikeltext ist inzwischen von der Seite verschwunden). Aue-Bad Schlema (Beitrag der Stadt auf ihrer Facebook-Seite, mit Link — gefunden nicht über die Facebook-Suche, sondern über eine Websuche nach unserer Adresse neben „Platz 1"). und Urmitz (öffentlicher LinkedIn-Beitrag eines Beteiligten vom Umweltfonds, 20.08.; war zwischendurch zurückgesetzt, weil ihn weder Besucherstatistik noch LinkedIn-Suche zeigten — gefunden hat ihn die Quer-Suche).
  - **Keine unserer Quellen hat Berkenthin und Riedstadt gefunden, gefunden hat sie eine Websuche nach Ort und Aufhänger.** Regionalzeitungen stehen weder auf der Gemeinde-Domain (die Suchmaschinen-Abfrage je Gemeinde sieht sie nicht), noch sind sie schnell genug im Verweis-Verzeichnis, und ohne Klick gibt es keine Besucherherkunft. Ein Bericht über Veröffentlichungen fragt deshalb zusätzlich die Websuche je angeschriebenem Ort nach Ort, „Platz 1" und der Messgröße des Briefes (`npm run kommunen:presse`, schreibt nichts: jeder Treffer ohne Link ist ein Kandidat, bis jemand die Seite gelesen hat).
  - **Jede belegte Veröffentlichung ist eine eigene Zeile** (Tabelle `kommunen_veroeffentlichung`, Zählung `lib/kommunen-veroeffentlichung.ts`, Eintragen und Bilanz `npm run kommunen:veroeffentlichungen`). Der Status an der Gemeinde sagt nur ja/nein; wie viele Beiträge, wie viele mit Link und wie viel woanders als auf der Gemeindeseite steht, war nur durch Lesen aller Notizen zu beantworten. Das Eintragen setzt Status und Notiz mit, ein Schreibweg. **Die Quote rechnet mit den Briefen ohne bekannten Zustellfehler.** Kommandozeile (`kommunen:stand`) und die Übersicht im Admin-Bereich lesen dieselbe Zählung. Stand 22.09.2026: 7 von 285 Gemeinden (2,5 %), 11 Beiträge, 10 mit Link, 7 woanders; je Schub 6,4 % (He/RP/SL, der älteste), 2,1 % (Nord/Ost), 0 % (Ni/HB, NRW).
  - **Die Hinweise kommen seit dem 22.09.2026 von selbst, einmal die Woche** (`.github/workflows/kommunen-hinweise.yml`, montags): Besucherherkunft und Websuche laufen mit `--melden` und legen NUR NEUE Hinweise ab — neu heißt, die Fundstelle steht noch in keiner Notiz der Gemeinde (`lib/kommunen-hinweise.ts`, festgenagelt von `lib/__tests__/kommunen-hinweise.test.ts`). **Erledigt ist ein Hinweis, sobald sein Ergebnis in der Notiz steht, belegt oder verworfen** — eine eigene Tabelle „gesichtet" wäre eine zweite Wahrheit neben der Notiz. Die offenen Hinweise zeigt `npm run kommunen:stand`, auch leer („nichts Neues"), damit ein ausgefallener Lauf nicht wie ein ruhiger aussieht. Der Anlass: Berkenthin und Riedstadt standen am 22.09. schon in der Besucherstatistik, als „noch nicht eingeordnet", und blieben liegen, weil niemand sie dort ansah.
  - **Das Wort „Solar-Check" ist kein Fund.** Kelkheim, Eppelborn, Langenhagen und die Wedemark standen seit dem 30.08. mit „Verweis gefunden" bzw. „Text übernommen" in der Tabelle; gemeint war jedes Mal der Solar-Check der Verbraucherzentrale bzw. der Klimaschutzagentur. Die Suche prüft seitdem auf unsere Domain, die alten Einträge blieben aber stehen, bis sie am 22.09.2026 korrigiert wurden — **wer eine Erkennung verschärft, räumt auch auf, was die alte schon geschrieben hat.** **Was die Messung nicht sieht, wird benannt:** Eine App-Plattform schickt keinen Verweis mit, deshalb steht Wallertheim in der Ausgabe als „vermerkt, hier nicht sichtbar" — sonst läse sich die Zahl als vollständig.
  - **Eingeordnet wird nur, was beim Namen genannt ist.** Alles andere heißt „andere Seite" und wird zum Ansehen ausgegeben; eine unbekannte Domain als Veröffentlichung zu zählen wäre dieselbe Erfindung wie ein Prüfdatum ohne Prüfung. **Ohne Verweis wird nicht gedeutet** — darin steckt der Klick in der Mail genauso wie der Aufruf aus einer App. Und ein **Mail-Prüfdienst** (Trend Micro, Safe Links) öffnet Links automatisch: als Empfänger-Klick gezählt hätte er die Quote geschönt, seine Regel läuft deshalb VOR der Postfach-Regel. `lib/__tests__/outreach-herkunft.test.ts` hält jeden dieser Fälle an einem echten Verweis fest.
  - **Wer den Beitrag geschrieben hat, sagt der Verweis NICHT.** „Kam über Facebook" trennt die Gemeinde nicht von einem Bürger, der geteilt hat. Für die Quote zählt trotzdem, dass etwas veröffentlicht wurde; wer die Gemeinde als Urheber behaupten will, sieht den Beitrag an.
  - **Der Vermerk trägt KANAL UND DATUM, und sagt, woher er das weiß** (`--schreiben`). „Über Facebook" ist in vier Wochen nicht mehr einzuordnen, ein Datum ohne Kanal sagt nicht, woran wir es gesehen haben — und beides zusammen wäre ohne den dritten Teil eine Behauptung: Genannt wird der **erste Besuch von dort**, nicht der Tag der Veröffentlichung, den wir nicht kennen. **Ein Beitrag ist EIN Kanal:** Facebook meldet sich je nach Gerät als vier verschiedene Hostnamen, im ersten Vermerk standen deshalb vier „Kanäle" für eine Veröffentlichung. Und der Vermerk benutzt **nicht** die Form der Rückläufer — die sagt „aus Postfach", und hier hat niemand geschrieben.
...
- **WER die Aufbauten verursacht, zeigen die Request-Logs, nicht das Protokoll-Werkzeug** (19.09.2026). Die gruppierten Laufzeit-Protokolle kennen keinen Aufrufer; die Request-Logs des Dashboards (`vercel.com/api/logs/request-logs`, Token der Vercel-Kommandozeile) tragen `clientUserAgent`, Referer und Region. **Das Umblättern dort ist unzuverlässig** — stundenweise Fenster nehmen, das Ende auf den ältesten Zeitstempel der letzten Antwort setzen und nach `requestId` entdoppeln, sonst zählt man dieselben fünfzig Zeilen sechzigmal. Erster Befund damit: Der Sprung vom 18.09. war vor allem PerplexityBot auf den Atlas-Ortsseiten — ein verifizierter Crawler, am Bot-Schutz bewusst ausgenommen. Nicht gesperrt (zitierende KI-Suche), Wiedervorlage am 26.09.2026; hält der Durchlauf an, wird er auf den Ortsseiten per Firewall gedrosselt, nicht gesperrt.
...
**Die 404-Seite gibt es ZWEIMAL, und das ist keine Doppelung — BLOCKER (20.09.2026).** Eine falsche Adresse erreicht den Besucher auf zwei Wegen, und Next rendert sie aus zwei Dateien: `app/global-not-found.tsx` für eine Adresse, auf die **keine** Route passt, und `app/(site)/not-found.tsx` für eine Seite, die es gibt, hinter der aber nichts steht (ein erfundener Ort im Atlas). Bis dahin stand an beiden Stellen Next' nackte Standardseite — richtiger Statuscode, aber eine Sackgasse ohne Menü und ohne Fußzeile.
- **Eine gewöhnliche `not-found.tsx` unmittelbar in `app/` ist hier nicht möglich, und der Grund kommt wieder.** Dieses Projekt hat dort **kein** Wurzel-Layout: `(site)`, `(embed)` und `(partner)` bringen jeweils ihr eigenes mit. Eine `not-found.tsx` eine Ebene darüber hat damit kein Layout über sich, und Next weist sie ab — gemessen **HTTP 500 auf jeder unbekannten Adresse**, nicht etwa ein Rückfall auf die Standardseite. `global-not-found.tsx` ist genau dafür da: Sie IST das Dokument, `html` und `body` eingeschlossen.
- **Sie hängt an einem Schalter** (`experimental.globalNotFound` in `next.config.js`). Ohne ihn ignoriert Next die Datei vollständig: Sie bleibt liegen, die alte nackte Seite kommt zurück, und nichts wird rot. Der Schalter ist per Test festgenagelt.
- **Geteilt ist alles, was ein Besucher SIEHT — nicht nur der Wortlaut.** Die Worte stehen in `lib/nicht-gefunden.ts`, die Zeichnung in `components/NichtGefundenInhalt.tsx`, ihr Aussehen in `public/shared-404/nicht-gefunden.css`; beide Seiten rendern denselben Baustein. Zweimal gebaut driften sie in Wortlaut UND Aussehen, und keine der beiden sähe für sich falsch aus (dieselbe Systematik wie Brief und Gemeindeseite). Den Rahmen nimmt die globale Seite aus `lib/neon-unterseite.ts`.
- **Das Stylesheet wird VERLINKT, nicht importiert, und jede Regel ist gekapselt.** Ein Import landet im CSS-Bündel **jeder** React-Seite — gemessen, die Regeln standen auf dem Impressum und in jedem Rechner; eine 404 hat auf Seiten, die keine ist, nichts zu kosten. Und weil die Datei dort neben den globalen Stilen der Site liegt, steht jeder Selektor unter `.sc-nf`: eine Regel auf ein nacktes `h1`, `main` oder `a` färbte die ganze Site ein, und das fiele nicht an der 404 auf, sondern irgendwo anders. Beides per Test festgenagelt.
- **Was übrig bleibt, ist der allgemeine Alt/Neu-Unterschied der Site, kein 404-Problem:** Die globale Seite ist eine Dokumentseite des neuen Designs (fester heller Grund), die Atlas-Variante steckt im React-Rahmen, der der Tagesstufe folgt. Das verschwindet, wenn die Atlas-Seiten auf das neue Design umziehen — es hier zu erzwingen hieße, als einzige React-Seite die Tagesstufe zu ignorieren.
- **Bekannte Grenze, gemessen und bewusst offen gelassen:** Im Browser-Reiter der Atlas-Variante steht der Titel des Seitenrahmens statt „Seite nicht gefunden". Eine not-found-Datei kann keine Metadaten mitgeben, und ein gerenderter Titel verliert gegen den, den das Framework selbst schreibt. Zu beheben wäre das nur mit einem Eingriff in JEDER Seite, die eine 404 werfen kann — das Gegenteil der einen Quelle. Die Seite steht ohnehin auf „nicht indexieren".
- **Beide behalten HTTP 404.** Eine Weiterleitung auf die Startseite ist die verbreitete Abkürzung und genau der Schaden — Google sieht dann eine gültige Seite. Der Gesundheitscheck fragt seit dem 20.09.2026 **beide** Wege ab (die erfundene Atlas-Adresse und eine Adresse ohne jede Route); `lib/__tests__/nicht-gefunden.test.ts` hält die Struktur, vor dem Einchecken fünfmal absichtlich kaputtgemacht.
- **Was der Atlas-Weg NICHT liefert, und das ist älter als diese Arbeit:** Unter `/solar-atlas/…` steht die 404-Seite nur im nachgeladenen Teil, der ausgelieferte Rumpf ist leer — mit Next' Standardseite genauso (am 20.09.2026 gegen einen Bau ohne unsere Seite gemessen). Der Statuscode stimmt, die Seite baut sich erst im Browser auf.
...
Drei Sorten:
- **Flow-Tests** (7) klicken die Hauptflows durch — bei jedem Push **jede Option jedes Schritts und jeden Zweig** (eigener CI-Job, EIN Playwright-Arbeiter, Produktions-Build), **alle Kombinationen nächtlich** (`flows-nightly.yml`, `FLOW_ALLE_KOMBINATIONEN=1`). **Nächtlich läuft jeder Flow in einem EIGENEN Job** (seit 07.09.2026): Der gemeinsame Lauf stand bei 258 von 280 erlaubten Minuten und riss das Limit am 05.09.; zwei Flows tragen 86 % davon (PV-Rechner 1.728 Wege / 174–182 min, Wärmepumpe 1.120 / 100–107 min, die übrigen fünf zusammen unter 50 min — **an den abgeschlossenen Einzeljobs** nachgemessen am 16.09.2026; die erste Fassung las die Startzeitpunkte der Tests im gemeinsamen Job und lag mit 149 bzw. 71 min um ein Fünftel bis die Hälfte zu niedrig). Getrennte Jobs kürzen die Gesamtzeit auf rund 150 Minuten, **ohne einen einzigen Weg wegzulassen** — das Limit anzuheben wäre der Zug, den das Wächter-Gate verbietet, und weniger Kombinationen wären weniger Abdeckung. **Der Lauf hat DREI Zeitgrenzen, und die innerste war die engste** (24.09.2026): Test-Limit 195 min (`FLOW_TEST_ZEITLIMIT_MIN` in `e2e/flows.ts`) < Schritt-Limit 210 < Job-Limit 230. Beim Aufteilen am 07.09. bekamen Schritt und Job Luft für die gemessenen 182 Minuten, das Test-Limit blieb bei den 3 Stunden vom 18.08.2026 — der PV-Job war damit drei Nächte rot (21.–23.09.), jedes Mal **exakt 3 h 0 m nach dem Teststart** und jedes Mal mit einer Meldung, die auf einen Produktfehler zeigte („Weiter kam nicht durch“) statt auf einen Zeitablauf, weil `weiterKlicken` den Abbruch in seinem `catch` fängt. Wer hier wieder ein zu knappes Limit vermutet, **vergleicht zuerst den Zeitpunkt des Fehlers mit dem Teststart**; ein Retry war es nicht, nächtlich steht `retries` auf 0. Die Reihenfolge der drei Grenzen hält `lib/__tests__/workflow-schritt-zeitlimits.test.ts` fest. **Nicht mit mehr Arbeitern verwechseln:** Die teilen sich einen Server und erzeugten am 18.08.2026 genau die Fehlalarme, gegen die `--workers=1` steht; getrennte Jobs teilen nichts. **Die Aufteilung wird aus `FLOWS` ABGELEITET** (`scripts/flow-matrix.ts`, `npm run flows:matrix`), nie in die Workflow-Datei getippt: Ein Job, dessen Auswahlausdruck ins Leere greift, wird nicht rot, sondern **grün** — Playwright meldet „keine Tests" und der Job ist durch. Deshalb prüft der erste Job jede Nacht gegen Playwright, dass jeder Teil mindestens einen Test bekommt und alle zusammen den Bestand genau einmal abdecken; genau diese Gegenprobe hat beim Bauen den eigenen Fehler gefangen (`--grep` vergleicht mit dem ganzen Pfad „[flows] › datei › Titel", ein `^` am Titelanfang trifft deshalb nie — der Sammel-Job wäre still doppelt gelaufen). Nicht zurück auf „jede Kombination bei jedem Push" bauen — gemessen und verworfen, der Runner sättigt und meldet dann Fehler, die keine sind. Zustandsändernde Klicks laufen über `waehle`/`weiterKlicken` (`e2e/flows.ts`): klicken, Zustandswechsel nachweisen, sonst wiederholen — ein blinder Klick auf einen `aria-disabled`-Knopf wird stumm verschluckt.
...
**Ein Klick auf einen React-Knopf wird NACHGEWIESEN, nicht gehofft — BLOCKER (gemessen 20.09.2026).** Ein servergerenderter Knopf ist vollständig anklickbar, bevor React ihn übernommen hat: sichtbar, ruhig, nicht gesperrt, nichts darüber. Playwrights Prüfung auf Bedienbarkeit ist damit erfüllt und **kann den fehlenden Handler nicht sehen** — der Klick wird stumm verschluckt, und die Seite sieht dabei völlig normal aus. Gefangen an der Geschichten-Karte, mit einem Zeitstempel für den Klick und einem für den Augenblick, in dem der Knopf seinen Handler bekommt: grüner Lauf Klick bei 295 ms gegen Handler ab 263 ms, roter Lauf Klick bei 206 ms gegen Handler ab 293 ms. Es sind fünfzig bis hundert Millisekunden — genug, dass der Test mit EINEM Arbeiter fast immer grün ist und mit ZWEIEN zu 7 von 8 Läufen rot, weil beide Browser dieselben Skriptpakete gleichzeitig holen.
- **Das sieht wie ein Parallelitätsproblem aus und ist keins.** Wer es dafür hält, heftet den Test an einen Arbeiter oder schaltet Wiederholungen ein — beides lässt die Ursache stehen und gewöhnt daran, Rot nicht ernst zu nehmen.
- **LÄNGER WARTEN HILFT NICHT, und das ist gemessen:** ein einzelner Klick mit zwanzig Sekunden Wartezeit darauf ließ dieselben zwei Prüfungen rot, 23 Sekunden je Lauf. Ein verlorener Klick ist verloren; das Ereignis hatte nie einen Empfänger. Nur ein zweiter Klick holt ihn zurück.
- **Der Baustein dafür ist `klickBisWirkung` (`e2e/klick.ts`)**: klicken, die Wirkung als sichtbares Element nachweisen, sonst noch einmal — dieselbe Einsicht, aus der `waehle` und `weiterKlicken` entstanden sind, nur ohne Flow-Bezug. **Erst prüfen, dann klicken:** Hat der erste Klick gewirkt, liegt das Fenster über dem Knopf, und ein blinder zweiter Versuch liefe ins Zeitlimit, obwohl alles in Ordnung ist.
- **Die Nachbarn sind nur zufällig grün.** Sie laden mit dem späteren Signal (`load` statt `domcontentloaded`) und gewinnen das Rennen deshalb meist — gewonnen ist es damit nicht. Wer dort einen Klick ergänzt, nimmt denselben Baustein.
...
**Und nicht an Layout-Details, die man sehen MÜSSTE, um sie zu finden (05.09.2026).** Eine vierte Auswahlkarte und eine fünfte Tabellenspalte gingen ihm mit der Frage „passt das auf 375 px?" zur Abnahme. Seine Antwort: „ich kann das nicht testen. das muss ein system 100 % zuverlässig testen. wie soll mir ein fehler auffallen?" Ob etwas ein paar Pixel über den Rand ragt oder ob eine Option fehlt, entscheidet kein Blick, sondern eine Messung — seitdem prüft `e2e/kein-ueberlauf.spec.ts` jede Seite der Rundgangsliste auf Telefonbreite — seit 23.09.2026 auf **zwei** Breiten (375 und 320 px), weil ein Telefon die Seite bei Überlauf still breiter macht und der Fehler bei 375 dadurch genau dort verschwindet, wo er entsteht; zwei Seiten mit vier Pixeln Rest stehen mit Grund und Frist in der Ausnahmeliste. Die Auswahlkarten klickt der Flow-Läufer ohnehin. **Seine Abnahme bleibt dort, wo er etwas ENTSCHEIDET** — ob ein Feature so aussehen und so heißen soll —, nie dort, wo er einen Fehler entdecken müsste. Wer eine Änderung zur Abnahme schickt, sagt dazu, WELCHE Entscheidung sie ihm stellt; fällt einem keine ein, gehört an ihre Stelle ein Test.
...
haben sieben belegte Veröffentlichungen erzeugt (Stand 22.09.2026) und **null Einbettungen**; in der Galerie
...
- **Das Kontaktdatum wird NICHT zurückgesetzt** — die naheliegende Reparatur und die falsche: Daran hängt die Tagesmengen-Zählung des Versands, und der Zeitpunkt des ersten, gescheiterten Versuchs wäre für immer weg.
- **Ein volles Postfach bleibt liegen.** Dort stimmt die Adresse, es gibt nichts zu ersetzen, und in ein paar Tagen ist es wieder leer (Selzen). Nur die dauerhafte Unzustellbarkeit rechtfertigt eine neue Adresse.
- **GEZÄHLT WERDEN FEHLVERSUCHE, NICHT ADRESSEN** (`dauerhafteBouncer`). Die Obergrenze soll den Kreislauf „neue Adresse → prellt wieder → neue Adresse" beenden. Die erste Fassung zählte dafür die toten Adressen in der Notiz — und sperrte Lassan nach seinem EINZIGEN Fehlversuch: Dessen Zustellmeldung nennt zwei Adressen, weil unser Empfänger auf ein gelöschtes Personenpostfach in Wolgast weiterleitete und der Server beide meldet. Dieselbe Verdopplung entsteht bei jeder Alias-Erweiterung (Selzen: `info@` und `webmaster@` aus einer Meldung). **Gefunden nur, weil die Ausnahme gegen die vier echten Fälle lief statt gegen ausgedachte Notizen** — die trugen je eine Adresse und waren grün. Eine Handkorrektur zählt nicht mit; sie ist die Reparatur eines Fehlversuchs, nicht ein zweiter.
...
- **Eine Antwort auf eine SACHFRAGE ist keine Antwort auf den Brief — BLOCKER (10.09.2026).** Dasselbe Postfach trägt zwei Gespräche, und beide gehen an dieselben Amtsadressen: Gemessen tragen **64 der 289 angeschriebenen Gemeinden ein Förderprogramm** im Katalog, im offenen Versandtopf 19 von 175 — der Absender allein unterscheidet dort nichts. Ohne Weiche verbucht der Lauf die Antwort einer Förderstelle als Reaktion auf unser Anschreiben, setzt den Status, schreibt den Zeitstempel und meldet eine Entscheidung, die es nicht gibt; verdorben wäre ausgerechnet die einzige Kennzahl, an der der Erfolg des Briefes hängt. Erkannt wird sie am Betreff der Anfrage (`istAntwortAufSachfrage`, Marke aus `INQUIRY_BETREFF_ANFANG` — **importiert, nie getippt**, sonst fällt die Weiche beim ersten Umformulieren stumm aus). **Aussortiert, aber gezählt und benannt:** Eine stumme Ausblendung wäre von einem leeren Postfach nicht zu unterscheiden. **Kein Präfix in eckigen Klammern als Merkmal** — das ist die Maschinen-Signatur, gegen die die Anfrage-Mail ausdrücklich gebaut ist. Beim Bauen gemessen und deshalb im Code: Ein zitierter Betreff, den das Mailprogramm umbricht, trägt mitten in sich ein „> " und blieb sonst unerkannt.
...
**Ob eine Ansprache gewirkt hat, wird GETAKTET NACH DEM VERSAND gemessen — für jeden Bestand (`npm run wirkung`, `lib/outreach-wirkung.ts`, Tabelle `outreach_wirkung`, nächtlich im Förder-Wächter).** Je Versandtag entsteht an **Tag 3, 7, 14 und 28** eine Zeile mit Angeschriebenen, Antworten, Verweisen auf uns und (nur bei Kommunen) Abo-Anmeldungen.
- **Der Kalender ist der falsche Takt.** Ein Monatslauf misst bei einem Schub vom 2. etwas anderes als bei einem vom 28. — und beantwortet die eigentliche Frage nicht, nämlich wie lange eine Reaktion auf sich warten lässt.
- **Eine gemessene Zahl wird NIE überschrieben:** Der Wert an Tag 7 ist eine Aussage über Tag 7, nicht über heute. Ein ausgefallener Lauf holt den Punkt nach, und ein nachgeholter sagt das im Bericht („nachgemessen am …") — sonst verkauft eine Nachmessung sich als Verlaufskurve.
- **Ohne Verweis-Abruf wird gar nichts geschrieben.** Eine Zeile „0 verlinken uns" wäre keine vorsichtige Schätzung, sondern eine erfundene Null, die niemand je wieder anfasst; der Messpunkt bleibt offen und kommt morgen wieder. Ist nichts fällig, wird nichts abgerufen und nichts bezahlt (sonst ein Abruf, ~0,03 $).
- **Der Anlass ist ein Zählfehler:** Nidda verlinkt uns mit einer eigenen Seite zum Balkonsolar-Programm und galt trotzdem nur als „geantwortet", weil Verweis-Prüfung und Stand zwei getrennte Dinge waren. Was die Messung NICHT sieht, steht an jeder Auswertung: Veröffentlichungen ohne Verweis (Print, App-Plattformen, soziale Netze).
...
**Kontakte erfassen ist EIN Ablauf für alle Bestände (20.09.2026).** `lib/kontakt-suche.ts` entscheidet Zuständigkeit, Rolle und Alt/Neu-Vergleich, `scripts/lib/kontakt-lauf.ts` führt bewerten, begrenzte Nachrecherche, Nachprüfung der Belegseite und Fortschritt. Was je Bestand verschieden ist, sind drei Angaben: welche Rollen gesucht werden, welche Domains als eigen gelten, welche Einträge es gibt (Muster: `KOMMUNEN_ROLLENWERK`, Anleitung `docs/erhebung/kontakte-neuer-bestand.md`). **Wer für Fachbetriebe, Versorger oder einen Presseverteiler eine zweite Erfassung baut, baut den Fehler, den dieses Projekt bei den Fachbetrieben schon einmal bezahlt hat.** Belegt ist die Gleichwertigkeit gemessen: Nach dem Herausziehen lieferten alle 10.747 Gemeinden zeichengleich dasselbe Ergebnis wie vorher.

**Eine Adresse geht erst hinaus, wenn sie drei Prüfungen besteht — für jeden Bestand (22.09.2026).** Ein Beleg sagt, wo eine Adresse steht, nicht wofür sie taugt: Die Erfassung fand auf genau den Seiten, die sie liest, Datenschutz-Postfächer, Vorlagen-Platzhalter („vorname.nachname@", „email@example.com") und die Adressen von Schlichtungsstelle und Webagentur im selben Impressum.
- **Taugt das Postfach?** `lib/kontakt-tauglichkeit.ts` ist die EINE Liste ungeeigneter Postfächer und Platzhalter; die Versandprüfung der Gemeinden liest sie mit, die Auswahl der Fachkontakte überspringt ungeeignete und nimmt die nächste belegte Adresse. **webmaster@ steht bewusst nicht darin** — bei kleinen Gemeinden das einzige Postfach und oft die Person, die ein Widget einbauen kann.
- **Nimmt die Domain Mails an?** Ohne Mailserver ist es eine falsch gelesene Adresse (angeklebter Text, Tippfehler im Impressum).
- **Steht sie heute noch auf der Fundstelle?** Frisch abgerufen, bei Seiten, die erst im Browser entstehen, mit einem echten Browser. Gemeinden: Nachprüfung im Versandlauf (höchstens drei Tage alt). Fachbetriebe, Versorger und Presse: `npm run kontakte:freigabe -- --bestand=…` (eine Prüfung für alle, `scripts/lib/kontakt-freigabe.ts`) schreibt Freigabedatum oder Sperrgrund an jeden Kontakt; **ein künftiger Versand an diese Bestände nimmt nur Adressen mit frischer Freigabe.**
- **Seiten, die ein einfacher Abruf nicht lesen kann, liest ein zweiter Durchgang mit echtem Browser** (`scripts/lib/kontakt-browser.ts`), ohne Tarnung. Gemessen an den Fachbetrieben: 159 von 308 Betrieben ohne Adresse hatten danach eine belegte.

**Empfänger kommen aus der Kontaktsuche der zweiten Generation (19.09.2026).** `scripts/contact-municipal-v2.ts` belegt je Gemeinde Klimaschutz- und Pressekontakte an der veröffentlichten Seite und trägt alle belegten in eigene Spalten der Kontaktliste ein (`--mode=apply --schreiben`); die allgemeinen Postfächer bleiben unberührt. **Klimaschutz ist der bevorzugte Kontakt** (Betreiber), dann Presse, dann Pressepostfach, dann allgemeines Postfach; eine belegte Person darf dabei einen Personennamen tragen (`lib/kommunen-fachkontakt.ts`, `postfachBefund` mit `belegteRolle`).
- **Die erste Generation ist als Sperre ausgebaut.** Sie verlangte vor jedem Versand, dass bundesweit keine Gemeinde mehr ungeklärt ist — 10.741 von 10.747 waren es, der Versand war damit dauerhaft gesperrt. Jetzt prüft der Versandlauf jeden Empfänger einzeln (`lib/contact-v2-gate.ts`): Ergebnis unter den aktuellen Regeln, Adresse in der geprüften Auswahl, und die Seite, auf der sie steht, unmittelbar vor dem Versand noch einmal abgerufen.
- **Das LESEN der Rückmeldungen macht der Wächter, nicht ein Mensch** (`scripts/kommunen-rueckmeldungen-verify.md`, Schritt 4i des zweistündlichen Förder-Wächters). Abholen und Nachtragen liefen längst automatisch; angesehen werden mussten die Reste trotzdem von Hand, und genau deshalb blieben sie liegen — Berkenthins Antwort neun Tage, zwei Veröffentlichungs-Hinweise drei Wochen. Jede nicht zuzuordnende Mail und jeder offene Hinweis wird im Lauf selbst geöffnet und entweder eingetragen oder mit Grund in der Notiz verworfen. **Über Antworten wird dabei NICHT gemeldet** (Betreiber, 23.09.2026): Sie liegen in seinem Postfach, weil der Brief genau dorthin zurückverweist — eine Mail „X hat geantwortet" wäre die zweite Nachricht über denselben Vorgang. Hinaus geht nur, was eine Folge hat, die er dort nicht sieht: ein erkannter Widerspruch, der die Gemeinde dauerhaft sperrt.
- **Jeder Versand speichert Adresse UND Art des Empfängers** (Klimaschutz, Pressekontakt, Pressepostfach, allgemeines Postfach). Die ersten 289 Briefe gingen ohne jeden Vermerk hinaus; ob Fachkontakte besser antworten als info@, ließ sich deshalb nur nachträglich aus heutigen Adressen erschließen. Die Art steht neben der Adresse, weil sich Adressen einer Gemeinde nach Rückläufern und neuen Suchen ändern.
- **Eine Regeländerung macht alle Ergebnisse ungültig**, bis sie neu ausgewertet sind (`--mode=evaluate`, offline) — der Versand hält sonst jeden Brief an.
