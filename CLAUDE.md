...
- **`/photovoltaik-foerderung`** + `/[bundesland]` + `/[bundesland]/[stadt]` — Förderdaten in Supabase (`funding_programs`, `funding_checks`) über `lib/funding-data.ts` mit Code-Seed als Fallback; ISR 3600, Rechner via `/api/funding`, Sync `/api/funding/setup?resync=1`. Runbook `scripts/foerder-verify.md`.
  ...
  **Ein Förderbetrag verfällt von selbst — BLOCKER, und die eigentliche Absicherung.** `fundingZaehlt()` (`lib/funding-programs.ts`) ist die **einzige** Stelle, die entscheidet, ob ein Programm Geld abziehen darf: Anträge offen **und** Quellenbeleg jünger als `FOERDER_MAX_ALTER_TAGE` (180 Tage — ein komplett ausgefallener Quartals-Zyklus ist gedeckt, zwei nicht). Vorher galt ein Abzug unbefristet, solange `status: "aktiv"` dastand; ob das noch stimmte, hing allein daran, dass ein Wächter lief und ihn widerrief — und die Wächter laufen nur, wenn der Rechner des Betreibers an ist (09.–13.08.2026 lief fünf Tage keiner). **Eine Zusage, die nur durchs Ausbleiben eines Widerrufs weitergilt, wird still falsch.** Jetzt ist die Beweislast umgedreht: Fällt die Prüfung aus, verfällt der Abzug ohne dass irgendetwas laufen muss. Seiten, Rechner und CTA fragen `fundingZaehlt()`, nie `status === "aktiv"` selbst — sonst bietet ein Knopf eine Förderung an, die der Rechner daneben nicht mehr abzieht. Gemessen bei Einführung: Von 38 Programmen ziehen genau **5** überhaupt Geld ab, alle im August 2026 belegt — die Regel änderte am Einführungstag nichts. **Ohne Datenbank (Seed-Fallback) gibt es gar kein Prüfdatum und damit keinen Abzug**; das ist gewollt. Festgenagelt von `lib/__tests__/funding-beleg-verfall.test.ts`.
