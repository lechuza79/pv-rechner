...
> **`/klimaanlage-stromkosten`** (Klimaanlagen-Rechner, Kühlkosten): eigener Flow (Gerätetyp → Räume/Größe → Nutzung/PLZ) → Ergebnis mit Gerätevergleich (Monoblock/mobile Split/fest installiert, SEER-getrieben), Anschaffung (je-Raum bzw. €/kW), CO₂ und PV-Deckung. Kühlbedarf weather-driven aus **Kühlgradstunden** (`/api/cooling-degree` → Open-Meteo Archiv, Supabase-Cache `klima_cache`, Bundesland-Fallback aus Config), akuter Hitzewellen-Blick aus 16-Tage-Vorhersage. Reine Funktionen `lib/aircon.ts`, Config `lib/aircon-config.ts` (Geräte/SEER/Preise/Klimatologie/Hitzeschwelle, `validFrom`/`reviewBy`, auf /datenstand). NUR Kühlung — Heizen via Wärmepumpen-Rechner.
