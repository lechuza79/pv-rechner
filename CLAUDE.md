...
**`components/Header.tsx` liegt noch im Baum, wird aber von keiner Seite mehr gerendert — und das ist ein offener Punkt, kein Zustand (20.09.2026).** Gelöscht ist sie nicht, weil `lib/__tests__/nav-aktiv.test.ts` sie als Quelle für die Menü-Markierung liest. Genau darin liegt das Problem: **Dieser Wächter prüft eine Datei, die niemand mehr ausliefert** — er ist grün, ohne etwas zu sehen, dieselbe Klasse wie ein Test, der das Vorhandensein statt der Verwendung prüft. Die neue Navigation ordnet einen Pfad über `navigationOwner` einer von fünf Gruppen zu und kennt die früheren Schlüssel je Seite gar nicht mehr; den Wächter darauf umzustellen ist keine Umbenennung, sondern die Frage, was die neue Zuordnung garantieren soll. **OFFEN (bis 11/2026):** entweder den Wächter auf `public/shared-nav/nav-content.js` umstellen und die Datei löschen, oder begründen, warum es ihn nicht mehr braucht.
