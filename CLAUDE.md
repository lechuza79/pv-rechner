...
- **`/photovoltaik-foerderung`** + `/[bundesland]` + `/[bundesland]/[stadt]` — Förderdaten in Supabase (`funding_programs`, `funding_checks`) über `lib/funding-data.ts` mit Code-Seed als Fallback; ISR 3600, Rechner via `/api/funding`, Sync `/api/funding/setup?resync=1`. Runbook `scripts/foerder-verify.md`.
  **„Zuletzt geprüft" darf nur eine echte Prüfung behaupten — BLOCKER.** Nur `last_verified` speist das Datum, **nie** `updated_at` als Ersatz: Das ist der Zeitpunkt der letzten Schreibung (z. B. eines Resyncs), bei der niemand etwas geprüft hat. Über diesen Fallback trugen bis zum 16.08.2026 **25 der 38 Programme** ein Prüfdatum für eine Prüfung, die nie stattfand (nur 13 hatten ein echtes) — und jeder Resync frischte es auf. Gemessen an Bonn: `last_verified` leer, `updated_at` 17.06.2026, auf der Seite stand „Zuletzt geprüft: 17.06.2026". Ohne echtes Prüfdatum steht jetzt der redaktionelle `stand` da; das ist die schwächere, aber ehrliche Aussage. Dieselbe Fehlerklasse wie am 28.07.2026, als ein Ein-Programm-Lauf allen 36 Programmen das Tagesdatum aufstempelte. Festgenagelt in `lib/__tests__/funding-data.test.ts → „Herkunft des Prüfdatums"`.
  **Ein Zustandswechsel wird mitgeschrieben, bevor er überschrieben wird — BLOCKER.** `funding_history` (Tabelle in `/api/funding/setup`, Vergleich + Leseseite `lib/funding-history.ts`, Anzeige `components/FundingHistory.tsx` auf der Kommunenseite) hält jeden Wechsel von Status, Konditionen, Bedingungen, Höchstbetrag und Berechtigung fest. Der Vergleich sitzt dort, wo der Zustand **wirklich** überschrieben wird: im Resync von `/api/funding/setup`, der eine korrigierte Programmliste in die Datenbank trägt. **Der tägliche Seiten-Wächter kann das nicht leisten** — er schreibt nur Fingerabdrücke und weiß, DASS sich eine Amtsseite bewegt hat, nie WAS; ein „von 200 auf 150 €/kWh" wäre dort erfunden. Wer einen zweiten Schreibweg auf `funding_programs.data` baut, führt ihn über `vergleiche()`, sonst verschwindet der Wechsel wieder spurlos. Drei Regeln, per Test festgenagelt (`lib/__tests__/funding-history.test.ts`): Das Datum heißt **`festgestelltAm`, nie „geändert am"** (wir kennen den Tag unserer Feststellung, nicht den des Ratsbeschlusses); der Vergleich hat **keine Uhr** — der Zeitpunkt wird hereingereicht, nie aus `new Date()` oder `updated_at` gezogen (dieselbe Fehlerklasse wie beim erfundenen Prüfdatum); und die **strukturierten Rechenwerte werden aufgehoben, aber nie angezeigt**, weil sie keine Einheit tragen und eine erfundene eine zweite Quelle neben dem redaktionellen Satz wäre. Der Abschnitt blendet sich aus, solange es keinen echten Wechsel gibt — ein Eintrag „seit Mai im Katalog" ist kein Verlauf, sondern auf 110 Stadtseiten dieselbe leere Zeile. **Gelöscht wird nie**, auch nicht bei eingestellten Programmen: Die Protokolle sind das Beweismittel für die „wesentliche Investition" hinter dem Datenbankherstellerrecht. Die Rückwärts-Recherche der Vorgeschichte ist bewusst **nicht** gemacht (Handarbeit ohne verlässliche Quelle) — die Seite behauptet deshalb nur, was wir seit Aufzeichnungsbeginn beobachtet haben.
  **Geblockte Träger werden nachgehalten, nicht vergessen — BLOCKER.** Jeder Prüfversuch wird protokolliert (`funding_checks`, Leseseite `lib/funding-verify-state.ts`, Schreibseite `npm run foerder:probe`), und **nur der Ausgang `traeger` zählt als geprüft**. Daraus entsteht der Arbeitsvorrat, mit dem jeder Lauf beginnt: erst die, an denen wir hängen, dann die ältesten. Vorher stand in den Wächter-Aufträgen „merke dir, welche du nur sekundär belegen konntest" — eine Anweisung, die keine Sitzung befolgen kann, weil jede bei null anfängt; dauerhaft geblockte Träger fielen deshalb still hinten runter. Das **Archiv der Amtsseite** ist eine eigene Stufe der Leiter, belegt aber nur den Inhalt, nicht die Aktualität, und setzt das Prüfdatum nicht zurück. Nach **drei** Läufen ohne Amtsquelle fällt das Programm auf `unsicher` (kein Abzug mehr) und die Entscheidung geht an den Betreiber; letzte Stufe ist eine sachliche Anfrage an den Träger (`lib/funding-inquiry-draft.ts`, **Entwurf, kein Versand**).
  **Ein Förderbetrag lebt von der BESTÄTIGUNG, nicht von einer Frist — BLOCKER.** `fundingZaehlt()` (`lib/funding-programs.ts`) ist die einzige Stelle, die entscheidet, ob ein Programm Geld abziehen darf. Erste Fassung war ein festes Höchstalter von 180 Tagen auf die inhaltliche Prüfung — vom Betreiber zurückgewiesen, zu Recht: „da haben wir ein halbes Jahr einen alten Stand auf der Seite." Die Frist war als Notbremse gedacht und wurde zum Ersatz für die Prüfung. **Die richtige Größe ist nicht das Alter, sondern ob wir den Stand gerade bestätigen können.** Der Seiten-Wächter ruft jede Amtsseite täglich ab; ist sie unverändert, gilt der geprüfte Inhalt weiter — dafür braucht es keine Frist, das ist schlicht wahr. Die Uhr läuft nur, wenn wir NICHT bestätigen können, und dann **zwei Wochen** statt sechs Monaten: nach einer erkannten Änderung (`FOERDER_NACHPRUEF_FRIST_TAGE`, Inhalt unbekannt) oder ohne geglückten Abruf (`FOERDER_BESTAETIGUNG_MAX_TAGE`, wir wissen nicht, ob sie sich geändert hat). Der Wächter läuft täglich, ein Programm hat also rund vierzehn Anläufe — wer die alle verfehlt, verfehlt sie nicht wegen einer Laune. Seiten, Rechner und CTA fragen `fundingZaehlt()`, nie `status === "aktiv"` selbst. **Ohne Datenbank (Seed-Fallback) gibt es keinen Abruf-Nachweis; dann zählt die inhaltliche Prüfung**, damit ein Datenbankausfall nicht schlagartig jede Förderung abschaltet. Festgenagelt von `lib/__tests__/funding-beleg-verfall.test.ts`.
  **Der Seiten-Wächter ist der einzige Teil der Prüfung, der ohne Modell und ohne den Rechner des Betreibers läuft** (`scripts/funding-watch.ts`, GitHub-Action `foerder-watch.yml`, täglich, kostenlos). Er ruft alle Amtsseiten ab und vergleicht einen Fingerabdruck des sichtbaren Texts; was sich bewegt hat, steht am nächsten Morgen oben im Arbeitsvorrat. **Grenzen, die man nicht verwischen darf:** Er weiß nicht, WAS sich geändert hat, und findet keine neuen Programme — beides braucht Urteilsvermögen und bleibt beim Modell-Lauf. **Ein grüner Lauf heißt „nichts hat sich bewegt", nicht „alles korrekt".** Und: **ein gescheiterter Abruf des Crawlers ist KEIN Fehlversuch** (eigene Kennung `seite-unerreichbar`), sonst eskaliert er falsch — gemessen am ersten Cloud-Lauf (17.08.2026) waren vom Rechner des Betreibers 2 Seiten unerreichbar, aus GitHubs Rechenzentrum 5; dieselben Seiten, andere IP-Reputation. Ein Fehlversuch entsteht erst, wenn die volle Leiter inklusive echtem Browser gescheitert ist. Die Beschriftung trennt beides strikt („hat sich geändert" vs. „Abruf kam nicht durch") — eine unerreichbare Seite als „geändert" auszuweisen behauptet eine Beobachtung, die es nicht gab.
  **Abgerufen wird aus der eigenen Produktion, nicht vom CI-Runner** (`/api/funding/fetch`, aufgerufen von `scripts/funding-watch.ts`). GitHub-Runner hängen an Azure-Rechenzentrumsadressen, die viele Anbieter pauschal sperren — gemessen am 17.08.2026: archive.org antwortet dort mit 503/523 auf jedem Weg, von einem normalen Anschluss mit 200. Die Vercel-Function in `fra1` ist dagegen eine Adresse, die echten Web-Verkehr ausliefert; sie holt die Seite, der Runner fragt nur nach dem Fingerabdruck. Wirkung im selben Lauf: drei zuvor unerreichbare Städte kamen durch. **Die Route ist KEIN offener Proxy** — sie nimmt eine Programm-Kennung, nie eine Adresse, löst die URL aus unserer Datenbank auf, sitzt hinter `CRON_SECRET` und gibt nur den Fingerabdruck zurück, nie den Inhalt. Der Fingerabdruck trägt seine Herkunft (`live:` / `archiv:`), damit ein Wechsel des Abrufwegs keine Änderung vortäuscht; die Normalisierung steht deshalb einmal in `lib/funding-fingerprint.ts`.
  **Nur ein LIVE gelesener Abruf bestätigt eine Seite — ein Archiv-Treffer nie.** Sonst bliebe eine dauerhaft gesperrte Stadt für immer „bestätigt", weil jede Nacht dieselbe wochenalte Kopie neu gestempelt wird — genau der alte Stand, den die 14-Tage-Regel verhindern soll. Wechselt der Abrufweg (live ⇄ archiv), ist kein Vergleich möglich; das wird als „nicht vergleichbar" ausgewiesen statt stillschweigend als „unverändert" verbucht.
  **Der Fingerabdruck entsteht aus Zahlen und langen Wörtern** (`lib/funding-fingerprint.ts`), nicht aus jedem Zeichen: wuerzburg.de verwürfelt seine Kontaktadresse als Spamschutz bei jedem Aufruf, ein zeichengenauer Abdruck meldete dort täglich eine Änderung — unter der 14-Tage-Regel wäre das Programm auf reines Rauschen hin dauerhaft aus der Rechnung gefallen. Beträge, Fristen und Fachwörter überleben die Verdichtung, Füllwörter und Buchstabensalat nicht. **Uhrzeiten werden bewusst NICHT gefiltert:** Eine Uhrzeit (09:14) und ein kurzes Datum (14.06.) sind per Muster nicht zu unterscheiden, und eine verpasste Antragsfrist ist der teurere Fehler. Beide Richtungen sind per Test festgenagelt (`lib/__tests__/funding-fingerprint.test.ts`).
  **Der Katalog soll VOLLSTÄNDIG werden, nicht stichprobenhaft** (Vorgabe des Betreibers, 18.08.2026). Vier Läufe arbeiten daran, alle nächtlich in `foerder-watch.yml`, in dieser Reihenfolge: **`foerder:ags`** prüft jeden Gemeindeschlüssel gegen das Melderegister · **`foerder:suche`** findet Förderseiten auf den Amtsdomains · **`foerder:seiten`** gleicht Fingerabdrücke aller bekannten Förderseiten ab · **`foerder:screen`** ordnet ein, was neu ist oder sich bewegt hat. Jeder legt sein Ergebnis ab und macht dort weiter, wo der letzte aufhörte — ohne Gedächtnis begänne jeder Lauf wieder bei den größten Städten und käme nie in die Tiefe.

  **Nicht der Kalender entscheidet über eine erneute Prüfung, sondern die BEWEGUNG.** Eine erste Fassung setzte ein festes Vierteljahr an; der Betreiber hat das zurückgewiesen — „ein Förderprogramm das 89 Tage den falschen Status hat wäre dumm". Dasselbe Argument hatte schon die 180-Tage-Frist beim Beleg-Verfall gekippt. Bewegt sich ein Fingerabdruck, steht die Gemeinde am selben Tag wieder im Screening-Lauf. Ein Datum bleibt nur als Netz für Seiten ohne Abdruck (`WIEDERVORLAGE_TAGE`, ein Jahr).

  **Ein achtstelliger Gemeindeschlüssel ist eine Zahl ohne Aussehen — BLOCKER.** Vertippt man sich um eine Stelle, bleibt er gültig und zeigt auf einen anderen Ort: kein Typfehler, kein roter Test, keine kaputte Seite, nur die falsche Gemeinde bekommt die Förderung angeboten. Am 19.08.2026 zeigten sechs von 43 neu aufgenommenen Programmen woandershin (Limburgerhof nach Neuhofen, Poing nach Moosach), und der erste Lauf über den GESAMTEN Katalog fand einen Altbestand: Bad Homburgs Programm hing an Glashütten — angeboten an 3.000 Einwohner im Taunus, vorenthalten den 57.000 in Bad Homburg. Niemandem aufgefallen, weil ein Test den falschen Schlüssel festgeschrieben hatte und grün war: Er verglich den Fehler mit sich selbst. **Ein Gemeindeschlüssel lässt sich gegen keinen Testwert absichern, nur gegen das Register** — deshalb `npm run foerder:ags`, täglich, darf rot werden. Schlüssel IMMER aus `mastr_regions` oder `funding_coverage` holen, nie aus einer Bildschirmliste.

  **Der Screener liest nicht, er sortiert vor** — er meldete eine Seite über Zuschüsse zu Verhütungsmitteln als PV-Treffer, weil das Wort im Navigationsmenü stand (behoben: `sichtbarerText` wirft Navigation, Kopf- und Fußbereich weg). Ein Eintrag entsteht erst, wenn jemand die Amtsseite wirklich gelesen hat; ein Screening-Zitat ist nie die Quelle für eine Zahl. Gemessen an einem Tag paralleler Prüfung: **grob die Hälfte der Fundstellen ist nichts** — Beratungsangebote, Dachbegrünung, Zuschüsse für Garagenhöfe, Landes- und Kreisprogramme in kommunalem Gewand.

  **Was ein Mensch gelesen hat, bleibt gelesen** (`gelesen_am` in `funding_coverage`, gesetzt über `foerder:screen -- --gelesen`). Der Screener stuft bei jedem Lauf neu ein und kennt kein Gestern; ohne dieses Gedächtnis stünde eine verworfene Seite morgen wieder oben, und eine Liste, die zur Hälfte aus Abgelehntem besteht, liest irgendwann niemand mehr. Eine Ausnahme, und sie ist der Grund für den Fingerabdruck oben: Bewegt sich die Seite, kommt auch ein gelesener Treffer zurück.

  **Die eigentliche Lücke ist die Trefferquote der Suche.** Gemessen am 19.08.2026: 9.722 Gemeinde-Websites durchsucht, nur 1.303 Förderseiten gefunden — 13 %. Sie geht zwei Klicks tief und sieht nur, was verlinkt ist. Bei realistisch 5–10 % Programmdichte fehlen mehrere hundert Programme. Der Engpass ist NICHT das Lesen: Vier bis sechs Prüf-Agenten parallel, je acht Seiten, sind erprobt und brachten an einem Tag 44 Programme. Übergabe mit allen Zahlen in Memory `project_foerder_abdeckung`.
  **Stadtseite und Katalog hängen an EINER Ableitung** (`fundingFor` / `fundingForFrom` in `lib/atlas-cities.ts`), nicht an einem handgepflegten `fundingId`. Zwei Listen, die man synchron halten MUSS, werden irgendwann nicht synchron gehalten: Herne und Ludwigshafen standen im Verzeichnis, ihre Programme im Katalog, die Seite sagte nichts — und Bremerhaven verpasste ein **aktives** Landesprogramm. Seiten, Bundesland-Übersicht, Sitemap **und Seitentitel** fragen dieselbe Funktion; der Titel lief zuletzt noch übers alte Feld und hätte für ein eingestelltes Programm „Zuschüsse" versprochen. `lib/__tests__/atlas-funding-sync.test.ts` hält die Gegenrichtung: kein Programm ohne Seite ohne ausgeschriebenen Grund mit Frist.
  **Der Ortsschlüssel entscheidet, wessen Bestand unter dem Ortsnamen steht — BLOCKER.** `ATLAS_CITIES` führt seit 19.08.2026 **fünf- ODER achtstellige** Schlüssel: fünf für eine kreisfreie Stadt oder einen Landkreis, acht für eine kreisangehörige Gemeinde. Der Atlas reicht den Schlüssel unverändert als Präfix durch, also setzt ein Kreisschlüssel unter einem Ortsnamen den Bestand des ganzen Kreises dorthin — bei Linsengericht 30.836 Anlagen statt 1.121. Der Fehler fällt niemandem auf, weil die Seite dabei völlig normal aussieht. Genau so lag es bei **Aachen, Hannover und Saarbrücken**, deren fünfstellige Schlüssel der StädteRegion, der Region und dem Regionalverband gehören: Die Hannover-Seite zeigte den Bestand von 1,14 Mio. Einwohnern unter dem Namen einer Stadt mit 522.000 (rund 57.000 statt 11.945 Anlagen). Drei Sicherungen: `npm run foerder:ags` prüft **Programme und Verzeichnis** täglich gegen das Melderegister, `atlas-funding-sync.test.ts` verlangt für jedes achtstellige Programm einen Eintrag mit achtstelligem Schlüssel, und die Seite schreibt den Nenner sichtbar an die Zahlen („nur Linsengericht, nicht Main-Kinzig-Kreis"). Jeder achtstellige Eintrag nennt zusätzlich seinen `kreis` — Mühlhausen und Senden gibt es mehrfach in Deutschland, und ohne den Kreis daneben liest man den Bestand des einen als den des anderen.

  **Der Standort-Ertrag im Verzeichnis ist gemessen, nicht geschätzt.** `yieldKwhKwp` kommt aus `/api/pvgis` an der repräsentativen Lage des Orts (`gemeindeGeo`), also aus derselben Quelle wie die Rechner. Bis 19.08.2026 standen dort Handwerte, und zwar in **104 von 105 Fällen zu niedrig**, im Mittel um 43 kWh/kWp — dieselbe Fehlerklasse wie der „Sicherheitspuffer" im bundesweiten Mittelwert, der einen Tag vorher entfernt wurde: Ein Abschlag gehört dorthin, wo die Angabe des Nutzers ihn begründet (die Dach-Matrix zieht ihn ohnehin ab), nicht in den Standortwert. Landkreis-Einträge behalten einen Handwert — ein Kreis hat keinen Punkt, an dem man messen könnte.

  **Prozentuale Zuschüsse tragen einen Deckel** (`percentOfCost` + `pvCap`). „20 % der Kosten, höchstens 300 €" ist die häufigste Bauform kommunaler Zuschüsse — ein Drittel der Fundstellen aus dem Screening —, und der Prozent-Zweig rechnete bis 18.08.2026 ungedeckelt. Solche Programme mussten deshalb ohne Betrag aufgenommen werden. **Was das Modell nicht ausdrücken kann, bekommt keinen strukturierten Satz** (z. B. „Sockel für die ersten 7 kWp, danach je kWp"): Das Programm informiert dann, zieht aber nichts ab — lieber keine Zahl als eine falsche.
  **Bot-Prüfungen werden nicht weggeklickt.** Gemessen am 16.08.2026 an frankfurt.de: direkter Abruf 403, skriptgesteuertes Chromium landet auf der Cloudflare-Prüfseite (unsichtbar **und** sichtbar gestartet, löst sich nicht auf), im echten Chrome erschien zeitweise das „Bestätigen Sie, dass Sie ein Mensch sind"-Häkchen — derselbe echte Browser lieferte die Seite eine Stunde später ohne jede Prüfung. Es ist also keine Mauer, sondern eine Laune: Ein einzelner Versuch ist Glückssache, über mehrere Läufe kommt man durch. Tarnwerkzeuge oder gelöste Mensch-Prüfungen sind keine Option — sie wären zusätzlich brüchig, und ein stillstehender Wächter meldet weiter Grün.
...
**Die Wächter hängen weiterhin am Rechner des Betreibers — Verlagerung in die Cloud ist ZURÜCKGESTELLT (15.08.2026, Kostengrund).** Gemessen an der Urlaubswoche: vom 09. bis 13.08.2026 lief **kein einziger** Wächter (keine `[auto]`-Commits, `lastRunAt` springt von 08.08. auf 13./15.08.), während die Health-Check-Action in derselben Zeit lückenlos alle drei Stunden weiterlief. Beim Zurückkommen feuerten die aufgelaufenen Läufe **gleichzeitig** (13.08.: vier in 31 Minuten; 15.08.: zwei in derselben Sekunde), jeder mit eigenem Bericht. Niemand hat die Lücke bemerkt — es gibt keinen Totmann-Schalter, der prüft, ob ein Wächter überhaupt noch meldet.
...
- **Alles Nötige liegt bereit:** `claude-autofix.yml` ist das erprobte Muster (Claude-Code-Action + `ANTHROPIC_API_KEY`), die Repo-Secrets (`ANTHROPIC_API_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `CRON_SECRET`) stehen, das Repo ist öffentlich → Actions-Minuten kostenlos. Die fachlichen Runbooks (`scripts/*-verify.md`) liegen ohnehin im Repo.
- **Zu verlagern wären die Wächter-Aufträge selbst.** Sie liegen ausschließlich unter `~/.claude/scheduled-tasks/*/SKILL.md` auf dem Rechner des Betreibers — elf Stück, sonst nirgends. Das ist unabhängig vom Urlaubsproblem ein Klumpenrisiko: Rechner weg = Aufträge weg. Beim Umzug gehören die Vercel-Projekt-/Team-IDs aus dem Triage-Prompt in Secrets (das Repo ist öffentlich).
- **Was in der Cloud NICHT ginge:** die letzte Stufe der Eskalationsleiter für bot-geblockte Träger-Seiten (`preview_start` + `get_page_text`, siehe `scripts/foerder-verify.md`). Genau diese Stufe hat bei Frankfurt drei Abweichungen gefunden, die keine Sekundärquelle kannte. Geblockte Programme müssten in der Cloud liegenbleiben und bei einem lokalen Lauf nachgeholt werden.
- **Kostenlage:** Heute laufen die Wächter über das Abo des Betreibers, also ohne Zusatzkosten; in der Cloud über den API-Schlüssel und einzeln abgerechnet. Geschätzt (nicht gemessen) 50–150 €/Monat auf dem großen Modell, 20–40 € wenn die zwei täglichen Läufe auf dem kleineren laufen. Treiber sind die täglichen Läufe, nicht die monatlichen.
...
**Ein roter Lauf muss unseren Code meinen — BLOCKER.** Der einzige CI-Schritt, der fremde Infrastruktur anfasst (`playwright install-deps`, also `apt-get` gegen Ubuntus Spiegel), ist deshalb `continue-on-error` mit 5-Minuten-Grenze. Am 18./19.08.2026 blieb er dreimal in 18 Stunden hängen: einmal 35 Minuten bis zum Job-Abbruch (**ein Lauf ohne Urteil ist schlimmer als ein roter** — „abgebrochen" liest man als „egal"), danach zweimal bis in die eigens dafür gesetzte Grenze. Jedes Mal Rot, ohne dass an unserem Code etwas war — und genau davon gewöhnt man sich ab, Rot ernst zu nehmen. Das Runner-Image bringt Chromiums Bibliotheken bereits mit; der Schritt ist Absicherung, kein Fundament. Der Browser-Download (Playwrights CDN, hat nie gehangen) bleibt fatal. Fehlt wirklich eine Bibliothek, scheitert der Testschritt danach mit einer lesbaren Browser-Meldung statt mit einem apt-Protokoll. Wer einen weiteren Schritt an fremden Servern hängen lässt, entscheidet dieselbe Frage neu: Darf deren Ausfall unsere Ampel umschalten?
...
- **Flow-Tests** (7) klicken die Hauptflows durch — bei jedem Push **jede Option jedes Schritts und jeden Zweig** (eigener CI-Job, EIN Playwright-Arbeiter, Produktions-Build), **alle Kombinationen nächtlich** (`flows-nightly.yml`, `FLOW_ALLE_KOMBINATIONEN=1`). Nicht zurück auf „jede Kombination bei jedem Push" bauen: ~600 Seitenaufbauten sättigten den 2-Kern-Runner, React übernahm frisch geladene Seiten >20 s nicht, und der Läufer meldete „Option ließ sich nicht wählen" für Optionen, die von Hand funktionieren — fünf rote Läufe am 18.08.2026, einer davon auf einem Commit, der nur CLAUDE.md änderte. Zustandsändernde Klicks laufen über `waehle`/`weiterKlicken` (e2e/flows.ts): klicken, Zustandswechsel nachweisen, sonst wiederholen — ein blinder Klick auf einen `aria-disabled`-Knopf wird stumm verschluckt.
...
**Wer einen Flow durchklickt, nimmt die Helfer aus `e2e/flows.ts`** (`uebrigeFragenBeantworten`, `waehle`) — nie eine eigene Kopie. Sie lagen bis zum 18.08.2026 in `flows.spec.ts` und standen damit nur dem Flow-Läufer zur Verfügung. Genau das ging schief: Als der Flow-Umbau die Vorauswahl aus jedem Schritt nahm, blieb „Weiter" gesperrt, bis **alle** Fragen des Schritts beantwortet sind (ein Schritt kann mehrere tragen, etwa Situation und Haustyp). Die älteren Tests klickten weiter nur „Weiter" und hingen am ausgegrauten Knopf — drei Browser-Tests standen stundenlang rot auf `main`, mit einer Ursache, die nichts mit dem zu tun hatte, was sie prüfen. Der Knopf sagt inzwischen selbst, was fehlt (`aria-label`), nur hört ihm ein Test nicht zu, der ihn nur anklickt.
