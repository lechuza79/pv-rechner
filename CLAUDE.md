...
- **Zwei Tabellen, `n:m`:** `utilities` (Name, `typ` stadtwerk/regionalversorger/genossenschaft, Kontakt, `sitz_gemeinde_id`, `status`, `notiz`) + `utility_communes` (`utility_id` × `commune_id`, `rolle` sitz/versorgungsgebiet/beteiligung, `zuordnung_quelle` verlinkt/recherchiert/vermutet). RLS **nur service_role** wie `kommunen_kontakt`. Anlegen: `scripts/utilities-refresh.ts --setup` (danach `NOTIFY pgrst` im Script — sonst „table not found in schema cache"), Bestand: `--stats`. **n:m von Anfang an**, weil ein Stadtwerk typisch 5–20 Gemeinden versorgt und deckungsgleiche Gebiete die Ausnahme sind.
...
- **Cockpit `/admin/versorger`** (Kachel in `/admin` + Sidebar): Liste mit Aufhänger + Gebiets-Kennzahlen, Filter Bundesland/Status/Name, Statuswechsel, Notiz; Detail-Fenster mit zugeordneten Gemeinden (Rolle + Herkunft, farblich hervorgehobenes „vermutet"), Platzierungen und Gemeinde-Suche zum Zuordnen. Tab **Erfassung**: die größten Gemeinden zuerst mit ihrer Website und dem Hinweis, ob dort schon ein Versorger hängt — das Stadtwerk steht meist auf der Gemeinde-Website verlinkt. **Kein Scraping**, solange die Menge klein ist. APIs: `/api/admin/utilities` (GET/POST/PATCH), `…/zuordnung` (GET Gemeinde-Suche, POST, DELETE), `…/erfassung` (Arbeitsliste).
...
- **muss ein Mensch anschauen** → Mail über `/api/alert`, Workflow rot.
