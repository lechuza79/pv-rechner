...
Einbettbare Widgets unter `app/(embed)/embed/*` (Strommix, Erzeugung, Karte, Simulation, Kennzahl, EE-Ampel, PV-Zubau, Einspeisevergütungs-Verlauf, **Förder-Check**, Amortisations-Rennen). Galerie mit Live-Vorschau + Copy-Paste-Code: `app/(site)/energie-widgets`. **Alle Widgets sind auf einem Stand — beim Bauen eines neuen dieselbe Konvention einhalten:**
...
- **Das Amortisations-Rennen (`components/charts/KostenrennenWidget.tsx`, Daten `lib/kostenrennen.ts`, Aufstellungen `lib/kostenrennen-varianten.ts`) rechnet mit der Amortisationsfunktion des Rechners, nie daneben.** Gezeichnet wird, was die Anlage EINGEBRACHT hat (kumulierter Nutzen aus `calc()` ab null), gegen eine Ziellinie „Anschaffung"; der Monat, in dem die Kurve sie erreicht, IST die Amortisation, per Test festgenagelt. **Ab null, nicht als Kosten:** In kumulierten Stromkosten startet der PV-Haushalt 14.000 € über dem Referenzhaushalt, und keine Achse zeigt dann den Winter (±60 €) — so stand es einen halben Tag da, bevor der Betreiber die Race-Chart-Vorlage zeigte, in der alle Läufer bei null starten. Die Stromkosten leben seitdem nur noch im Monatsstreifen unter dem Rennen. Drei Regeln aus dem Bau (05.09.2026): **Monatsauflösung kommt aus `calc().monate`**, das je Jahr exakt auf den Jahreswert summiert — kein zweiter Rechenweg für den Sägezahn (im Sommer nimmt der PV-Haushalt mit 10 kWp auf 3.800 kWh mehr ein, als seine Restrechnung kostet; die Kurve fällt dort, und das ist richtig). **Die Achsen laufen mit** (x bis heute, y bis knapp über das bisher Höchste), sonst ist die Jahresbewegung am Anfang ein Strich am Boden. **Rückblicke sind Beispiele, keine Prognosen:** Die Wetterjahre wiederholen die DWD-Strahlungsreihe (`lib/strahlungsjahre.ts`, erzeugt von `scripts/dwd-strahlung-sync.ts`) normiert auf das Mittel ihres Fensters — welche Strahlung unserem Referenzertrag entspricht, ist NICHT belegt, belegt ist nur die Streuung; die Preissprünge wiederholen die Eurostat-Reihe und laufen danach mit dem Modell weiter. Beide Aufstellungen stehen NEBEN dem Modell, nie darin, und das Rennen bleibt nominal (Betreiber, 05.09.2026) — in heutigen Euro nennte es einen anderen Gewinn als die Ergebnisseite.
...
npm run strahlung:sync # Gebietsmittel der Globalstrahlung je Jahr aus den DWD-Rastern (jährlich als Action; Wetterjahre im Stromkosten-Rennen)
