...
- **Das Stromkosten-Rennen (`components/charts/KostenrennenWidget.tsx`, Daten `lib/kostenrennen.ts`, Aufstellungen `lib/kostenrennen-varianten.ts`) rechnet mit der Amortisationsfunktion des Rechners, nie daneben.** Kosten mit Anlage = Kosten ohne Anlage − kumulierter Gewinn aus `calc()`; das Kreuzungsjahr IST die Amortisation, per Test festgenagelt. Drei Regeln aus dem Bau (05.09.2026): **Monatsauflösung kommt aus `calc().monate`**, das je Jahr exakt auf den Jahreswert summiert — kein zweiter Rechenweg für den Sägezahn (im Sommer nimmt der PV-Haushalt mit 10 kWp auf 3.800 kWh mehr ein, als seine Restrechnung kostet; die Kurve fällt dort, und das ist richtig). **Die Achsen laufen mit** (x bis heute, y bis knapp über das bisher Höchste), sonst ist die Jahresbewegung am Anfang ein Strich am Boden. **Rückblicke sind Beispiele, keine Prognosen:** Die Wetterjahre wiederholen die DWD-Strahlungsreihe (`lib/strahlungsjahre.ts`, erzeugt von `scripts/dwd-strahlung-sync.ts`) normiert auf das Mittel ihres Fensters — welche Strahlung unserem Referenzertrag entspricht, ist NICHT belegt, belegt ist nur die Streuung; die Preissprünge wiederholen die Eurostat-Reihe und laufen danach mit dem Modell weiter. Beide Aufstellungen stehen NEBEN dem Modell, nie darin, und das Rennen bleibt nominal (Betreiber, 05.09.2026) — in heutigen Euro nennte es einen anderen Gewinn als die Ergebnisseite.
