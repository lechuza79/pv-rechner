...
**Suche in der Kopfzeile (gebaut 24.09.2026, noch AUS):** Lupe neben dem Login, Klick öffnet ein Klappfenster mit dem Cursor im Feld; Treffer beim Tippen, Enter oder kein JavaScript führt auf `/suche` (noindex). Ein erkannter Ort (Name oder Postleitzahl) bekommt eine Karte mit Ortsseite, Förderung vor Ort und den Rechnern mit vorbelegter PLZ; alles andere wird gegen das Seitenverzeichnis gesucht. Eine Anfrage kann beides sein („Förderung Würzburg“) — Themenwort ist, was eine Seite trifft, der Rest ist Ort. Logik `lib/suche.ts`, Verzeichnis `lib/suche-verzeichnis.ts` (**gelesen** aus Menü und Ratgeber-Registry, nie eine dritte Liste; ergänzt werden nur Suchwörter je Pfad), Darstellung EINMAL in `public/shared-nav/search-content.js` für Klappfenster und Suchseite.
- **Der Schalter ist `SUCHE_IM_MENUE` in `public/shared-nav/nav.js`**, und er steuert auch den Link in der Ersatznavigation ohne JavaScript. Bis dahin erscheint die Lupe nur auf `/suche` und mit `?suchvorschau=1` (gilt für den Tab).
