...
export function werkzeugThema(html: string, url: string): WerkzeugThema {
  let pfad = url;
      pfad = entschluesseltOderRoh(url);
...
export function werkzeugAusSeite(html: string, url: string): Werkzeugbefund {
  ...
  const text = sichtbarerText(html);
  const pfad = entschluesseltOderRoh(url);
...
export const ZUSTAND_RANG: Record<WerkzeugZustand, number> = {
...
export const THEMA_RANG: Record<WerkzeugThema, number> = {
  waermepumpe: 6,
  solar: 5,
  balkon: 4,
  speicher: 3,
...
/**
 * Zustaende und Themen, staerkstes zuerst — ABGELEITET aus den Rangtabellen,
 * nie ein zweites Mal getippt. Jede Auswertung liest ihre Zeilen hieraus: Eine
 * getippte Liste faellt beim naechsten neuen Thema still auseinander, und was
 * dort fehlt, erscheint in keiner Zahl — genau so waren Balkonkraftwerk,
 * Wallbox und Speicher aus der Auswertung des Laufs verschwunden, waehrend die
 * Erkennung sie laengst fand.
 */
export const WERKZEUG_ZUSTAENDE = (Object.keys(ZUSTAND_RANG) as WerkzeugZustand[]).sort(
  (a, b) => ZUSTAND_RANG[b] - ZUSTAND_RANG[a],
);

export const WERKZEUG_THEMEN = (Object.keys(THEMA_RANG) as WerkzeugThema[]).sort(
  (a, b) => THEMA_RANG[b] - THEMA_RANG[a],
);

export function besterBefund(befunde: Werkzeugbefund[]): Werkzeugbefund {
  const rang = ZUSTAND_RANG;
  const themaRang = THEMA_RANG;
