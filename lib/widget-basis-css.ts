...
/**
 * Base styles of the embeddable widgets: the --widget-* tokens, the aliases
 * recycled site components read, the font-size scale and a few shared
 * keyframes. One source for the embed layout (a document of its own, `:root`)
 * and for pages that show the same widgets inline under a scoping class
 * (the new municipality page) — two copies would drift.
 */
...
export function widgetBasisCss(wurzel = ":root", koerper = "body"): string {
  const dokument = wurzel === ":root";
  return `
  ${dokument ? "*,*::before,*::after{box-sizing:border-box}\n  html,body{margin:0;padding:0}" : ""}
  ${wurzel}{
