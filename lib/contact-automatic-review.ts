...
export type AutomaticSource = {url:string; digest:string; observedAt:string; valid:boolean; readable:boolean; identityText?:string};
...
export function checkAutomaticContact(c:ContactCandidate,s:AutomaticSource,ctx:AutomaticContext){
  ...
  const block=c.roleEvidence?.text??'';
  ...
  const scopeExplicit=cardNames===1?identityPattern.test(normalized(block)):cardNames===0&&unambiguousPage&&identityPattern.test(normalized(s.identityText??''));
  ...
  if(!c.roleEvidence?.exclusiveAddress)reasons.push('contact-card-not-exclusive');
  const local=email.split('@')[0];
  const press=/^(?:presse|pressestelle|pressestab|kommunikation)$/.test(local)&&/\b(?:pressestelle|pressereferat|pressearbeit|pressekontakt|presse[- ]und öffentlichkeitsarbeit)\b/iu.test(block);
  const energy=/^(?:klimaschutz|klimaschutzmanagement|energieberatung)$/.test(local)&&/\b(?:klimaschutzmanagement|klimaschutzmanager|klimaschutzstelle|energieberatung)\b/iu.test(block);
  const channel=press?'publishing':energy?'energy':null;
  if(!channel)reasons.push('explicit-functional-mailbox-required');
  if(/\b(?:kreis|amt|verbandsgemeinde|samtgemeinde)\b/iu.test(negativeContext+' '+(s.identityText??'')))reasons.push('shared-authority-scope-needs-review');
  ...
  return {email,sourceUrl:s.url,sourceDigest:s.digest,observedAt:s.observedAt,originalSupported,
    functionSupported:reasons.length===0,channel:reasons.length===0?channel:null,
