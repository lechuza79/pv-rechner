...
it('uses the balcony badge with scope, denominator and honest initial status',()=>{
 ...
 expect(html).toContain('balcony-1-small.webp');expect(html).toContain('von 15 Kommunen');expect(html).not.toContain('Erstmals erfasst');expect(html).not.toContain('<details');expect(html).toContain('Alle Platzierungen');expect(html).not.toContain('Platz gehalten');expect(html).toContain(row.href);
});
...
it('does not reuse a podium badge for a top-100 position',()=>{
 ...
 expect(html).toContain('top-100.svg');expect(html).toContain('Platz 94');expect(html).not.toContain('-small.');
});
