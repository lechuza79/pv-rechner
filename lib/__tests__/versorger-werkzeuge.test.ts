...
describe("Die Auswertung zaehlt jedes Thema, das die Erkennung kennt", () => {
  // ANLASS, und der Fehler war bereits eingetreten: Die Ausgabe des Erhebungslaufs
  // fuehrte eine GETIPPTE Themenliste mit vier Eintraegen, waehrend die Erkennung
  // sieben kannte. Balkonkraftwerk, Wallbox und Speicher wurden erhoben, in die
  // Datenbank geschrieben — und erschienen in keiner einzigen Zeile der
  // Auswertung. Kein Typfehler, kein roter Test, keine kaputte Seite: nur eine
  // Zahl, die es nicht gab. Genau die Fehlerklasse "Beschriftung sagt etwas
  // anderes, als die Zahl misst", und genau der Grund, warum der Vollauf mit der
  // neuen Themen-Trennung nichts gezeigt haette.

  const quelle = readFileSync(new URL("../versorger-werkzeuge.ts", import.meta.url), "utf8");

  /** Die Union aus dem Quelltext lesen — NICHT aus der Rangtabelle. Sonst
   *  belegte die Tabelle sich selbst, und ein Thema, das in beiden fehlt, faende
   *  niemand. */
  const unionAus = (name: string) => {
    const block = quelle.match(new RegExp(`export type ${name} =([^;]+);`));
    if (!block) throw new Error(`Typ ${name} nicht gefunden`);
    // Die Blockkommentare ZWISCHEN den Alternativen muessen weg, bevor gelesen
    // wird — sie enthalten selbst Anfuehrungszeichen, und ohne diesen Schritt
    // liest der Helfer halbe Kommentarsaetze als Zustandsnamen. Genau daran ist
    // die erste Fassung dieses Tests gescheitert, und das war die richtige
    // Richtung: ehrlich rot statt falsch gruen.
    const ohneKommentar = block[1].replace(/\/\*[\s\S]*?\*\//g, "");
    const werte = [...ohneKommentar.matchAll(/"([^"]+)"/g)].map((m) => m[1]);
    if (werte.length < 2) throw new Error(`Union ${name} unplausibel klein`);
    return werte.sort();
  };

  it("die abgeleiteten Themen decken die Typ-Union vollstaendig ab", () => {
    expect([...WERKZEUG_THEMEN].sort()).toEqual(unionAus("WerkzeugThema"));
  });

  it("die abgeleiteten Zustaende decken die Typ-Union vollstaendig ab", () => {
    expect([...WERKZEUG_ZUSTAENDE].sort()).toEqual(unionAus("WerkzeugZustand"));
  });

  it("beide Listen stehen staerkstes zuerst", () => {
    // Die Reihenfolge ist die inhaltliche Aussage der Rangtabelle. Eine Ausgabe,
    // die alphabetisch sortiert, stellt den Wichtigsten irgendwohin.
    expect(WERKZEUG_THEMEN[0]).toBe("waermepumpe");
    expect(WERKZEUG_THEMEN[WERKZEUG_THEMEN.length - 1]).toBe("unbekannt");
    expect(WERKZEUG_ZUSTAENDE[0]).toBe("rechner-mit-leadfunnel");
    expect(WERKZEUG_ZUSTAENDE[WERKZEUG_ZUSTAENDE.length - 1]).toBe("keins");
  });

  it("der Erhebungslauf zaehlt ueber die Ableitung, nicht ueber eine getippte Liste", () => {
    // Geprueft wird die VERWENDUNG: dass die Ableitung importiert dasteht,
    // beweist nichts — sie muss in den Schleifen stehen, die die Zeilen
    // erzeugen.
    const lauf = readFileSync(new URL("../../scripts/versorger-erhebung.ts", import.meta.url), "utf8");
    expect(lauf).toMatch(/for \(const t of WERKZEUG_THEMEN\)/);
    expect(lauf).toMatch(/for \(const z of WERKZEUG_ZUSTAENDE\)/);
    // Und die Gegenrichtung: keine handgetippte Aufzaehlung von Themen mehr,
    // ueber die gezaehlt wird.
    expect(lauf).not.toMatch(/for \(const t of \[/);
    expect(lauf).not.toMatch(/for \(const z of \[/);
  });
});
