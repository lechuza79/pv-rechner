...
async function main(){
 ...
 for(const city of cities){
  try{
   ...
   for(const topic of buildStoryPool(report).topics)for(let index=0;index<topic.observations.length;index++){
    ...
    if(template){
     const copy=storyCopy(report,topic.observations[index],story);
     if(!copy)throw Error(`Missing central copy: ${story.label}`);
     if(/NaN|undefined|Infinity/.test(copy))throw Error(`Invalid copy: ${story.label}`);
     if(copy!==story.teaser)throw Error(`Central copy differs: ${story.label}`);
     totals[template]=(totals[template]??0)+1;templates[template]=(templates[template]??0)+1;}
 ...
 await writeFile('scripts/.cache/story-templates/copy-audit.json',JSON.stringify({cities:result.length,totals,unmapped,errors,results:result},null,2));
