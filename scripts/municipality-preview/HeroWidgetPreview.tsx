...
export default function HeroWidgetPreview(){
 ...
 return <a href="/atlas-design-preview/index.html#atlas-data" target="_parent" onClick={event=>{event.preventDefault();parent.postMessage({type:'atlas-hero-open-monitor'},location.origin);}} aria-label="Zum Energiemonitor" onMouseEnter={()=>setPaused(true)} onMouseLeave={()=>setPaused(false)} onFocus={()=>setPaused(true)} onBlur={()=>setPaused(false)} className="hero-monitor-link"><div className={`${foundation.foundation} municipal-data sc-dashboard monitor-hero`} data-story-scheme="dark">
  {widget==='live'?<CurrentPower compact/>:item?<article className="hero-story">
   ...
   <div className="hero-story-visual">{item.story.solarMonth?<MonitorMonthlySolarChart data={item.story.solarMonth} compact autoPlay paused={paused}/>:<MunicipalChart story={item.story as any} compact/>}</div>
  </article>:null}
 </div></a>;
}
