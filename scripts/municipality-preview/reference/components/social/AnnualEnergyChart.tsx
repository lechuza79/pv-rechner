...
export function AnnualEnergyChart({data,compact=false}:{data:EnergyYear;compact?:boolean}){
 ...
 {!compact&&<header><h2>Solar und Wind: Niddas Energiejahr {data.year}</h2><p>Referenzmodell · Bestand Ende {data.year} · Stand {formatStoryDate(data.sourceDate)}</p></header>}
 <svg viewBox="0 0 520 520" role="img" aria-label={`Solar und Wind in ${data.town}, ${data.year}. Modellierte Tageserträge; Wind mit vereinfachter Referenzkurve.`}>
 {[0,maximum/2,maximum].map((v,i)=><circle key={v} cx="260" cy="260" r={82+v/maximum*145} fill="none" stroke="var(--atlas-text)" strokeOpacity=".15" strokeDasharray={i%2===0?'2 5':undefined}/>)}
 {Array.from({length:12},(_,month)=>{const d=new Date(Date.UTC(data.year,month,1)),i=(d.getTime()-Date.UTC(data.year,0,1))/86400000;const p=point(i,maximum*1.14);return <text key={month} x={p[0]} y={p[1]+4} textAnchor="middle" className={styles.label}>{new Intl.DateTimeFormat('de-DE',{month:'short',timeZone:'UTC'}).format(d)}</text>;})}
 ...
 <div className={styles.legend}><span><i/>Solar</span><span><i/>Wind</span></div>
 ...
 <div className={styles.controls}><button aria-label="Vorheriger Tag" onClick={()=>shift(-1)}><IconChevronLeft size={12}/></button><select aria-label="Tag auswählen" value={selected??''} onChange={e=>setSelected(e.target.value===''?null:Number(e.target.value))}><option value="">Ganzes Jahr</option>{data.days.map((d,i)=><option key={d.date} value={i}>{formatStoryDate(d.date)}</option>)}</select><button aria-label="Nächster Tag" onClick={()=>shift(1)}><IconChevronRight size={12}/></button><button aria-label="Tagesauswahl aufheben" onClick={()=>{setSelected(null);setHover(null);}}><IconRefresh size={14}/></button></div>
 <footer>Quelle: <a href={data.sourceUrl} target="_blank" rel="noreferrer">Open-Meteo · ERA5</a> und Marktstammdatenregister</footer></>}
 </div>;
}
