...
function CurrentPower(){
 ...
 <div className="monitor-native-chart">{failed?<p>Wetterdaten derzeit nicht verfügbar.</p>:!reading?<p>Wetterdaten werden geladen …</p>:<MastrLiveRadial energietraeger="solar" installedKwp={kwp} injected={points} highlightTs={current?.ts} secondaryBars unit="MW" bare fuelltBreite/>}</div></WidgetFrame>;
}
function LocalMap(){
...
function MonitorWidget({item}:{item:any}){
 ...
 return <WidgetFrame title={role.title} kind={role.kind} className={chart.visualTheme} data-story-scheme="dark" settingsPlacement={isDonut?"below-title":"header"} context={isDonut?undefined:<>Letztes Update: {formatDate(item.story.sourceDate??data.sourceDate)}{!isDonut&&item.template!=='anlagenraster'&&<> · {item.story.period}</>}</>} settings={isDonut?<WidgetSetting label="Zeitraum des Anlagentyps" stepper hideLabel size="md" value={period} onChange={setPeriod} options={[{value:'current',label:'Heute'},...history.observations.slice(0,13).map(row=>({value:row.end,label:new Date(row.end+'T12:00:00').toLocaleDateString('de-DE',{month:'long',year:'numeric'})}))]}/>:undefined} help={isDonut?<p>Frühere Monatswerte zeigen die heute erfassten Anlagen nach ihrem Inbetriebnahmedatum. Nachmeldungen können frühere Werte verändern.</p>:undefined}>
  {isDonut?<ShareDonut values={values}/>:<div className="monitor-widget-body"><MunicipalChart story={item.story as StoryConcept}/></div>}
 </WidgetFrame>;
}
...
export default function MunicipalDataPreview(){
 ...
 const sections=['Anlagenbestand','Zubau','Strom und Wert'];
 ...
 return <div ref={root} className={`${foundation.foundation} municipal-data sc-dashboard`} data-story-scheme="dark">
  ...
  {sections.map(section=>{
   const items=data.charts.filter(item=>item.section===section);
