...
import periods from './monitor-periods.json';
...
export function MonitorWidget({item}:{item:any}){
 ...
 const valuePeriods=periods.monthly.filter(row=>'value' in row);
 const chosenValue=valuePeriods.find(row=>row.month===period)??valuePeriods[0];
 const renderedStory=isValuation&&chosenValue?.value?{...item.story,title:(item.template==='electricity-value'?'Wert des Solarstroms':'Einspeisevergütung')+' · '+new Date(chosenValue.month+'-15T12:00:00').toLocaleDateString('de-DE',{month:'long',year:'numeric'}),period:chosenValue.month,values:[{...item.story.values[0],value:item.template==='electricity-value'?chosenValue.value.euro:chosenValue.value.feedInEuro}]}:item.story;
 ...
 const compositionStory=isComposition&&selected?{...item.story,countComparison:{...item.story.countComparison,total:selected.solarCount,selected:selected.solarCounts[segment]},values:[item.story.values[0],{...item.story.values[1],value:selected.solarKwp?selected.solarMix.find(row=>row.label===(segment==='gebaeude'?'Gebäudeanlagen':'Balkonkraftwerke'))!.value/selected.solarKwp*100:0}]}:item.story;
 return <WidgetFrame title={role.title} kind={role.kind} className={chart.visualTheme} data-story-scheme="dark" settingsPlacement={hasStockPeriod||isValuation?"below-title":"header"} context={isDonut||['energy-year','radial','electricity-value','feed-in-value','anlagenraster'].includes(item.template)?undefined:<>Letztes Update: {formatDate(item.story.sourceDate??data.sourceDate)}{!isDonut&&item.template!=='anlagenraster'&&item.template!=='energy-year'&&<> · {item.story.period}</>}</>} settings={hasStockPeriod?<WidgetSetting label="Zeitraum des Anlagenbestands" stepper hideLabel size="md" value={period} onChange={setPeriod} options={[{value:'current',label:'Heute'},...history.observations.slice(0,13).map(row=>({value:row.end,label:new Date(row.end+'T12:00:00').toLocaleDateString('de-DE',{month:'long',year:'numeric'})}))]}/>:isValuation?<WidgetSetting label="Monat der Berechnung" hideLabel stepper value={chosenValue.month} onChange={setPeriod} options={valuePeriods.map(row=>({value:row.month,label:new Date(row.month+'-15T12:00:00').toLocaleDateString('de-DE',{month:'long',year:'numeric'})}))}/>:undefined} help={hasStockPeriod?<p>Frühere Monatswerte zeigen die heute erfassten Anlagen nach ihrem Inbetriebnahmedatum. Nachmeldungen können frühere Werte verändern.</p>:isValuation?<p>Wetter und Inbetriebnahmen des gewählten Monats, bewertet mit den gespeicherten Preis- und Eigenverbrauchsannahmen vom 19. Sept. 2026. Nur vollständig berechenbare Monate sind auswählbar. Modellwerte, keine tatsächlichen Einnahmen.</p>:['radial','energy-year'].includes(item.template)?<p>Die Auswahl enthält nur vollständig vorhandene Wetterzeiträume. Jahresprofile verwenden den zum Jahresende rekonstruierten heutigen Anlagenbestand; stillgelegte Anlagen fehlen. Modellierte Erzeugung, keine Messung.</p>:undefined}>
  {isDonut?<ShareDonut values={values}/>:item.template==='anlagenraster'?<div className="monitor-widget-body"><MonitorComposition story={compositionStory}/></div>:<div className="monitor-widget-body"><>{item.story.energyYear?<MonitorAnnualEnergyChart data={item.story.energyYear} datasets={periods.annual}/>:item.story.solarMonth?<MonitorMonthlySolarChart data={item.story.solarMonth} datasets={periods.monthly.map(row=>row.solar)}/>:<MunicipalChart story={renderedStory as StoryConcept}/>}</></div>}
 </WidgetFrame>;
}
