export function mountGlobalNav(header,{active='',homeHref='/',atlasHref='https://solar-check.io/solar-atlas',calculatorsHref='https://solar-check.io/rechner-uebersicht',onWaitlist,artBase=new URL('./illustrations',import.meta.url).href.replace(/\/$/,'')}={}){
 ...
 const tool=(image,title,description,actions,related='')=>`<article class="sc-nav-tool"><div class="sc-nav-art"><img src="${artBase}/${image}-neon.webp" alt="" width="160" height="120"></div><div><h3>${title}</h3><p>${description}</p><div class="sc-nav-tool-actions">${actions}</div>${related}</div></article>`;
 ...
 nav.insertAdjacentHTML('beforeend', '<div class="sc-nav-actions"><a class="sc-nav-login" href="https://solar-check.io/login"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M15 3h5v18h-5M3 12h12m-4-4 4 4-4 4"/></svg>Einloggen</a><a class="sc-nav-contact" href="https://solar-check.io/kontakt"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="m3 6 9 7 9-7"/></svg>Kontakt</a></div>');
 ...
 const logo=document.createElement('img');logo.src=new URL('./logo-result.svg',import.meta.url).href;logo.width=140;logo.height=33;logo.alt='solar-check.io';menuBrand.append(logo);
