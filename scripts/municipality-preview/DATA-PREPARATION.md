...
| story-unit-value-build.ts | Unit inventory + weather + explicit old self-consumption assumption → unit-value-data and detailed audit | unitMonthValue per register unit | Requires explicit assumption; refuses existing version; Valuation date now uses the central German-calendar-day helper in the secured source branch; pristine archive retains the original UTC expression. |
