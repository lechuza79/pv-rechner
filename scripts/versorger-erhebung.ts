...
import { PARALLEL, holeSeite, imPool, sitemapAdressen } from "../lib/website-abruf";
...
async function schreibeBefund(
  ...
  if (e.abruf !== "ok") {
    ...
    if (error) throw new Error(`${k.name}: ${error.message}`);
    return;
  }
  const { data: existing, error: readError } = await db.from("utilities").select("werkzeug").eq("id", k.id).single();
  ...
  const { error } = await db
    ...
    .update({
      ...
      werkzeug: existing?.werkzeug?.sicherheit === "angesehen" ? existing.werkzeug : e.werkzeug,
  ...
  if (error) throw new Error(`${k.name}: NICHT gespeichert — ${error.message}`);
}
...
async function erhebe(k: Kandidat, stichtag: Date): Promise<ErhebungMitWerkzeug> {
  ...
  const hole = async (url: string) => {
    ...
    else failures.push(`${url}: ${seite.fehler}`);
  };
...

  {
    ...
    ziele.push(...alleAdressen.filter((u) => SOLARSEITE_MUSTER.test(entschluesseltOderRoh(u))).sort((a, b) => a.length - b.length).slice(0, 3));
    ...
    if (kennzeichnungOffen) {
      const direkt = alleAdressen.filter((u) => {
        try {
          return KENNZEICHNUNG_MUSTER.test(entschluesseltOderRoh(u));
        } catch {
    ...
    if (kontaktOffen) {
      const kontakt = alleAdressen.filter((u) => KONTAKT_MUSTER.test(entschluesseltOderRoh(u))).sort((a, b) => a.length - b.length);
  ...
  // Server.
  {
    ...
    if (!zwischen.postfaecher.some(p => istBrauchbar(p.art)) && !zwischen.kontaktformular) {
      const formular = suchFormular(start.html, basis);
      if (formular) {
        for (const wort of ["Impressum", "Kontakt"]) {
          await hole(suchAdresse(formular, wort));
        }
      }
    }
  }
  ...
  return { ...erhebung, werkzeug, abruf: failures.length ? "teilweise" : erhebung.abruf, fehler: failures.length ? failures.join("; ").slice(0,1000) : erhebung.fehler };
}
...
async function main(): Promise<void> {
  ...
  // Der deutsche Kalendertag, nicht der der Weltzeit: Zwischen Mitternacht und
  // zwei Uhr steht dort noch der Vortag, und --fortsetzen würde dann alles
  // überspringen, was gestern erhoben wurde — mitten in der Nacht, wo niemand
  // hinsieht.
  const seit = fortsetzen ? new Date(`${heuteInBerlin()}T00:00:00+00:00`) : null;
  ...
  log(`  ─ irgendein Weg ins Unternehmen     : ${z((e) => e.postfaecher.some(p => istBrauchbar(p.art)) || e.kontaktformular)}`);
  log(`  ─ gar kein Weg gefunden             : ${z((e) => !e.postfaecher.some(p => istBrauchbar(p.art)) && !e.kontaktformular)}`);
  log("");
  log("");
  log("── Werkzeug auf der eigenen Website ───────────────────");
  // Beide Listen kommen aus den Rangtabellen, nicht von hier. Die getippte
  // Themenliste an dieser Stelle nannte vier Themen, waehrend die Erkennung
  // sieben kannte: Balkonkraftwerk, Wallbox und Speicher wurden erhoben und in
  // KEINER Zeile ausgewiesen — die Zahl, fuer die der Lauf ueberhaupt laeuft,
  // war unsichtbar.
  for (const z of WERKZEUG_ZUSTAENDE) {
