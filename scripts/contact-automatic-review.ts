...
function currentState(directory:string,workflow:string,t:any){
 const filename=hash(t.dataset+':'+t.organization_id)+'.json',obs=resolve(directory,'supplemental',t.organization_id,'observations');
 const paths=[t.audit.inputPath,resolve(directory,'results',filename),resolve(workflow,'decisions',filename),resolve(workflow,'cases',filename),resolve(directory,'reviews',filename),...(findingsIndex.get(t.organization_id)??[]),resolve(workflow,'refresh-holds',filename),...(existsSync(obs)?readdirSync(obs).filter(f=>f.endsWith('.json')).sort().map(f=>resolve(obs,f)):[])];
...
function main(){
 ...
 const output=resolve(workflow,'automatic'),records=resolve(output,'records'),cache=resolve(output,'sources');
 for(const p of [records,cache])mkdirSync(p,{recursive:true,mode:0o700});
 ...
 for(let index=part;index<scope.items.length;index+=parts){
  const row=scope.items[index],id=row.inventoryOrganizationId,t=targets.get(id);if(!t)throw Error('Missing inventory target');
  try{
  ...
  for(const o of observations){
   ...
   if(existsSync(cachePath)){parsed=read(cachePath);const {payloadDigest,...payload}=parsed;if(hash(JSON.stringify(payload))!==payloadDigest)throw Error('Cached extraction integrity failed');if(parsed.engine!==engine||parsed.digest!==o.digest||parsed.url!==o.url)throw Error('Cached original identity mismatch');}
   ...
   for(const c of parsed.candidates)contacts.push(checkAutomaticContact(c,{url:o.url,digest:o.digest,observedAt:o.observedAt,valid,readable:parsed.readable,identityText:parsed.identityText},{name:input.name,domain,asOf:observedAt,heldEmails:input.heldEmails??[]}));
  ...
  const checked=[...grouped].map(([email,evidence])=>({email,originalSupported:evidence.some(c=>c.originalSupported),functionSupported:!held&&evidence.some(c=>c.functionSupported)&&!evidence.some(c=>c.reasons.some((r:string)=>['published-address-conflict','held-contact','excluded-or-conflicting-purpose','explicit-role-withdrawal','conflicting-municipal-scope'].includes(r))),evidence}));
  ...
  const comparison={selectedEmails,newlySupported:selectedEmails.filter(email=>!baseline.has(email)),retainedSupported:selectedEmails.filter(email=>baseline.has(email)),baselineNotSelected:[...baseline].filter(email=>!selectedEmails.includes(email))};
