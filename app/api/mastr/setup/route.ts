...
export async function GET(req: NextRequest) {
  ...
  // Die vier heissen Funktionen kommen aus lib/mastr-region-sql.ts — EINE Quelle
  // fuer Setup-Route und Apply-Skript. Sie lagen frueher doppelt im Repo, und ein
  // Setup-Lauf haette die langsamere Kopie kommentarlos zurueckgeschrieben.
  const { error: e2b } = await supabase.rpc("exec_sql", { sql: MASTR_REGION_FUNCTIONS_SQL });
  ...
  // 2d-3. Breite Award-Grundtabelle: je Gemeinde die Leistung/Kapazität jeder
  //        Erzeugerart, getrennt nach Träger (privat/gewerbe). Grundlage für den
  //        Kommunen-Solar-Award (Sieger je Kategorie × Rolle × Ebene). Wie
  //        mastr_gemeinde_solar eine schmale, vorberechnete Tabelle (~11k Zeilen)
  //        aus EINEM DB-seitigen Lauf — NIE live über die 562k-Rohzeilen (hat die
  ...
  const { error: e2d3b } = await supabase.rpc("exec_sql", {
    sql: `SELECT mastr_refresh_gemeinde_award();`,
  });
  results.push({ step: "mastr_gemeinde_award_refresh", status: e2d3b ? "error" : "ok", error: e2d3b?.message });
