...
import { EMBED_HERKUNFT_DDL } from "../../../../lib/embed-herkunft";

// Einmalige Einrichtung der Einbettungs-Zählung (embed_herkunft + Zähl-Funktion).
...
export async function GET(req: NextRequest) {
  ...
  const { error } = await supabase.rpc("exec_sql", { sql: EMBED_HERKUNFT_DDL });
  if (error) {
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
  return NextResponse.json({ ok: true, table: "embed_herkunft" });
}
