...
export async function POST(req: NextRequest) {
  ...
  type Zeile = {
    ...
    mastr_regions: { name: string; population: number | null } | { name: string; population: number | null }[];
  };
  const zeilen: Zeile[] = [];
  for (const prefix of bl) {
    for (let from = 0; ; from += 1000) {
      const { data, error } = await serviceDb
        ...
        .select(
          "region_id, website, kontakt_url, rollen_email, verwaltung_domain, outreach_status, kampagne, mastr_regions!inner(name, population)",
        )
  ...
  if (!dry) {
    ...
    for (let i = 0; i < auswahl.gewaehlt.length; i += 100) {
      const batch = auswahl.gewaehlt.slice(i, i + 100);
      for (const g of batch) {
        const { error } = await serviceDb
          .from("kommunen_kontakt")
          .update({ kampagne: KAMPAGNE, charge: g.charge, updated_at: now })
