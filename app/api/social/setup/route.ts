...
export async function GET(req: NextRequest) {
  ...
  const { error } = await supabase.rpc("exec_sql", { sql: SOCIAL_KONTEN_DDL + SOCIAL_PRUEFUNG_DDL });
  if (error) {
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
  return NextResponse.json({ ok: true, tables: ["social_konten", "social_pruefungen"] });
}
