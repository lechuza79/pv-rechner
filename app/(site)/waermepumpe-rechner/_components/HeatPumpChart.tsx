    ...
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", display: "block" }}>
      ...
      {yTicks.map(val => (
        <g key={val}>
          ...
          <text x={P.l - 8} y={y(val)} textAnchor="end" dominantBaseline="middle" fontSize={fsPx("--font-size-micro")} fill="var(--color-text-muted)" fontFamily="var(--font-chart-number)" style={{ fontWeight: 700 }}>{(val / 1000).toFixed(0)}k</text>
        </g>
      ))}
      {xTicks.map(i => (
        <text key={i} x={x(i)} y={H - 4} textAnchor="middle" fontSize={fsPx("--font-size-micro")} fill="var(--color-text-muted)" fontFamily="var(--font-chart-number)" style={{ fontWeight: 700 }}>{YEAR + i}</text>
      ))}
      {scenarios.map(s => {
          ...
          <g key={s.id}>
            ...
            {be && (
              <>
                ...
                <text x={x(be.i)} y={y(be.kum) - 11} textAnchor="middle" fontSize={fsPx("--font-size-caption")} fontWeight="700" fill={s.color} fontFamily="var(--font-chart-number)" style={{ fontWeight: 700 }}>{be.i}J</text>
              </>
            )}
          </g>
        );
